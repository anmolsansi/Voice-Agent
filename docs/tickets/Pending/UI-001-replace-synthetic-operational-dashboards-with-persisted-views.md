# UI-001: Replace synthetic operational dashboards with persisted views
...
**Depends on:** [SCH-003](SCH-003-claim-due-work-fairly-with-leases-and-concurrency-limits.md), [WRK-002](WRK-002-build-staff-queue-assignment-acknowledgment-and-resolution.md), [VOI-003](VOI-003-persist-confirmed-answers-and-late-transcript-enrichment.md)

**Blocks:** [UI-002](UI-002-complete-patient-and-call-review-screens.md), [RPT-001](RPT-001-replace-fixed-reports-and-make-exports-safe.md)
...
Replace synthetic operational dashboards with persisted views. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Prototype summaries and operational call representations overlap; a plausible dashboard does not prove durable scheduling or staff work.

Operational counts and rows must come from canonical patients, occurrences, attempts and work items using the same filters and clinic timezone.
...
This ticket closes the gap through: replace synthetic operational dashboards with persisted views. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Operational counts and rows must come from canonical patients, occurrences, attempts and work items using the same filters and clinic timezone. Separate synthetic demo pages visibly. Cursor pagination and explicit loading/empty/error/stale states are required; 503 must not show fixture data.
...
Ticket-specific boundary: Operational counts and rows must come from canonical patients, occurrences, attempts and work items using the same filters and clinic timezone. Separate synthetic demo pages visibly. Cursor pagination and explicit loading/empty/error/stale states are required; 503 must not show fixture data.

## 7. Facts, assumptions and unknowns

- **Fact:** Prototype summaries and operational call representations overlap; a plausible dashboard does not prove durable scheduling or staff work.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** SCH-003, WRK-002, VOI-003
...
Deliver replace synthetic operational dashboards with persisted views and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Directory: `app`](../../../app)
- [Directory: `src/modules/calls`](../../../src/modules/calls)
...
### Step 1: Map each dashboard field to its canonical query

**ACTION:** Map each dashboard field to its canonical query.

**WHY:** The required outcome is: every displayed count has an owner and timestamp.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Every displayed count has an owner and timestamp. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Implement authorized bounded summary and list queries

**ACTION:** Implement authorized bounded summary and list queries.

**WHY:** The required outcome is: list and totals share filter semantics.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** List and totals share filter semantics. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Connect navigation, filters and empty/error states

**ACTION:** Connect navigation, filters and empty/error states.

**WHY:** The required outcome is: staff can move from a count to its matching records.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Staff can move from a count to its matching records. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Remove fixture fallbacks from operational imports

**ACTION:** Remove fixture fallbacks from operational imports.

**WHY:** The required outcome is: database absence remains visible.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Database absence remains visible. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Every displayed count has an owner and timestamp.
- [ ] Step 2: List and totals share filter semantics.
- [ ] Step 3: Staff can move from a count to its matching records.
- [ ] Step 4: Database absence remains visible.

## 16. Error and failure semantics

Compare query filters, timezone and cursor boundary before blaming frontend caching.
...
### TEST-UI-001-01: No rows versus outage

**Purpose:** Prove no rows versus outage under the target contract.
...
**Action:** Exercise `No rows versus outage` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Empty data and503 display distinct states without samples. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Compare query filters, timezone and cursor boundary before blaming frontend caching.

### TEST-UI-001-02: Restart persistence

**Purpose:** Prove restart persistence under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Restart persistence` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** The same committed work remains visible after server restart. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Compare query filters, timezone and cursor boundary before blaming frontend caching.

### TEST-UI-001-03: Filter parity

**Purpose:** Prove filter parity under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Filter parity` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Summary counts equal the filtered durable list across page boundaries. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Empty data and503 display distinct states without samples.
- [ ] The same committed work remains visible after server restart.
- [ ] Summary counts equal the filtered durable list across page boundaries.
...
Hide broken summaries while preserving authenticated durable lists and work queue.
...
Notify the implementation owner of UI-002, RPT-001 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
