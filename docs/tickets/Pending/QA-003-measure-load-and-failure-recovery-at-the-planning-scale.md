# QA-003: Measure load and failure recovery at the planning scale
...
**Depends on:** [OPS-004](OPS-004-establish-an-isolated-synthetic-staging-environment.md), [QA-002](QA-002-prove-one-complete-controlled-real-call-workflow.md)
...
Measure load and failure recovery at the planning scale. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

1000 attempts/day,20 concurrent conversations and50 staff are planning targets, not measured capacity.

Measure a realistic arrival curve with retries included in1000 attempts,20 aggregate voice streams and50 active staff.
...
This ticket closes the gap through: measure load and failure recovery at the planning scale. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Measure a realistic arrival curve with retries included in1000 attempts,20 aggregate voice streams and50 active staff. Use synthetic media/provider simulation for sustained load and a capped approved real-provider sample. Record VM shape, latency/error/queue lag, CPU/RAM, database locks and reconnect behavior. Initial API target p95<1s excluding long exports/provider work must be reviewed before acceptance.
...
Ticket-specific boundary: Measure a realistic arrival curve with retries included in1000 attempts,20 aggregate voice streams and50 active staff. Use synthetic media/provider simulation for sustained load and a capped approved real-provider sample. Record VM shape, latency/error/queue lag, CPU/RAM, database locks and reconnect behavior. Initial API target p95<1s excluding long exports/provider work must be reviewed before acceptance.

## 7. Facts, assumptions and unknowns

- **Fact:** 1000 attempts/day,20 concurrent conversations and50 staff are planning targets, not measured capacity.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** OPS-004, QA-002
...
Deliver measure load and failure recovery at the planning scale and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
### Step 1: Define workload and approved service thresholds

**ACTION:** Define workload and approved service thresholds.

**WHY:** The required outcome is: average daily volume is not mistaken for peak load.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Average daily volume is not mistaken for peak load. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Exercise normal and burst traffic with secure password hashing

**ACTION:** Exercise normal and burst traffic with secure password hashing.

**WHY:** The required outcome is: memory and pool budgets remain bounded.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Memory and pool budgets remain bounded. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Inject database/provider/worker/proxy failures

**ACTION:** Inject database/provider/worker/proxy failures.

**WHY:** The required outcome is: recovery respects lifecycle, idempotency and consent.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Recovery respects lifecycle, idempotency and consent. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Publish bottlenecks and retest corrected limits

**ACTION:** Publish bottlenecks and retest corrected limits.

**WHY:** The required outcome is: claimed capacity has reproducible evidence.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Claimed capacity has reproducible evidence. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Average daily volume is not mistaken for peak load.
- [ ] Step 2: Memory and pool budgets remain bounded.
- [ ] Step 3: Recovery respects lifecycle, idempotency and consent.
- [ ] Step 4: Claimed capacity has reproducible evidence.

## 16. Error and failure semantics

Distinguish provider rate limits from VM saturation using dispatch and database telemetry.
...
### TEST-QA-003-01: 20 media plus50 staff

**Purpose:** Prove 20 media plus50 staff under the target contract.
...
**Action:** Exercise `20 media plus50 staff` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** No unbounded buffer, dropped committed work or authorization bypass. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Distinguish provider rate limits from VM saturation using dispatch and database telemetry.

### TEST-QA-003-02: Worker killed after send

**Purpose:** Prove worker killed after send under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Worker killed after send` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Reconciliation prevents duplicate outbound call. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Distinguish provider rate limits from VM saturation using dispatch and database telemetry.

### TEST-QA-003-03: Database outage

**Purpose:** Prove database outage under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Database outage` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** 503 responses and rollback occur; recovery does not invent success. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] No unbounded buffer, dropped committed work or authorization bypass.
- [ ] Reconciliation prevents duplicate outbound call.
- [ ] 503 responses and rollback occur; recovery does not invent success.
...
Lower admitted concurrency/volume and pause pilot until measured capacity meets agreed scope.
