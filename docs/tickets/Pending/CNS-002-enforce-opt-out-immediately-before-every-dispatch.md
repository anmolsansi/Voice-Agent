# CNS-002: Enforce opt-out immediately before every dispatch
...
**Depends on:** [CNS-001](CNS-001-persist-consent-and-communication-preference-history.md)

**Blocks:** [SCH-003](SCH-003-claim-due-work-fairly-with-leases-and-concurrency-limits.md), [TEL-003](TEL-003-dispatch-cancel-and-reconcile-uncertain-provider-outcomes.md), [REL-001](REL-001-accept-the-complete-post-visit-check-in-release.md)
...
Enforce opt-out immediately before every dispatch. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Canonical attempt creation currently checks schedule ownership/status but not an authoritative patient/guardian consent service.

One eligibility service protects scheduled, manual and retry dispatch.
...
This ticket closes the gap through: enforce opt-out immediately before every dispatch. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
One eligibility service protects scheduled, manual and retry dispatch. Lock/check current eligibility when claiming dispatch. Opt-out atomically suppresses pending work and records audit; request cancellation for already accepted active calls outside the transaction. Record uncertain cancellation and expose it to staff. An external call already accepted cannot be falsely claimed undone.
...
Ticket-specific boundary: One eligibility service protects scheduled, manual and retry dispatch. Lock/check current eligibility when claiming dispatch. Opt-out atomically suppresses pending work and records audit; request cancellation for already accepted active calls outside the transaction. Record uncertain cancellation and expose it to staff. An external call already accepted cannot be falsely claimed undone.

## 7. Facts, assumptions and unknowns

- **Fact:** Canonical attempt creation currently checks schedule ownership/status but not an authoritative patient/guardian consent service.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** CNS-001
...
Deliver enforce opt-out immediately before every dispatch and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `src/modules/consent/eligibility.js`
- `tests/dispatch-consent.integration.test.cjs`
...
### Step 1: List all current and proposed dispatch paths

**ACTION:** List all current and proposed dispatch paths.

**WHY:** The required outcome is: no simulator/manual route can become a live bypass.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No simulator/manual route can become a live bypass. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Centralize active patient/contact/authority/consent checks

**ACTION:** Centralize active patient/contact/authority/consent checks.

**WHY:** The required outcome is: each ineligible result has a safe stable reason.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Each ineligible result has a safe stable reason. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Commit opt-out and pending suppression together

**ACTION:** Commit opt-out and pending suppression together.

**WHY:** The required outcome is: new dispatch claims after revocation cannot pass.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** New dispatch claims after revocation cannot pass. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Handle active call cancellation and failed-provider cancellation visibly

**ACTION:** Handle active call cancellation and failed-provider cancellation visibly.

**WHY:** The required outcome is: durable suppression remains even if remote cancellation fails.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Durable suppression remains even if remote cancellation fails. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: No simulator/manual route can become a live bypass.
- [ ] Step 2: Each ineligible result has a safe stable reason.
- [ ] Step 3: New dispatch claims after revocation cannot pass.
- [ ] Step 4: Durable suppression remains even if remote cancellation fails.

## 16. Error and failure semantics

Track the provider-acceptance boundary. Do not promise impossible retroactive prevention or roll back local opt-out when cancellation fails.
...
### TEST-CNS-002-01: Opt-out versus worker race

**Purpose:** Prove opt-out versus worker race under the target contract.
...
**Action:** Exercise `Opt-out versus worker race` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** No later claim dispatches; accepted-call boundary and cancellation are recorded. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Track the provider-acceptance boundary. Do not promise impossible retroactive prevention or roll back local opt-out when cancellation fails.

### TEST-CNS-002-02: Retry/manual bypass

**Purpose:** Prove retry/manual bypass under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Retry/manual bypass` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Both reject revoked permission and create no provider side effect. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Track the provider-acceptance boundary. Do not promise impossible retroactive prevention or roll back local opt-out when cancellation fails.

### TEST-CNS-002-03: Provider cancel outage

**Purpose:** Prove provider cancel outage under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Provider cancel outage` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Local opt-out remains effective and reconciliation work is durable. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Track the provider-acceptance boundary. Do not promise impossible retroactive prevention or roll back local opt-out when cancellation fails.

### TEST-CNS-002-04: Restart
...
**Expected assertions:** Suppression persists independent of process memory. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] No later claim dispatches; accepted-call boundary and cancellation are recorded.
- [ ] Both reject revoked permission and create no provider side effect.
- [ ] Local opt-out remains effective and reconciliation work is durable.
- [ ] Suppression persists independent of process memory.
...
Pause all outbound dispatch if eligibility enforcement is uncertain; preserve revocation state and outstanding reconciliation.
...
Notify the implementation owner of SCH-003, TEL-003, REL-001 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
