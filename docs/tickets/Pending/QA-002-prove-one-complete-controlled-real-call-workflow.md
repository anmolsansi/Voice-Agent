# QA-002: Prove one complete controlled real-call workflow

**Status: Pending — Not completed**

**Priority:** P0 · **Area:** D — Telephony and controlled call · **Type:** Engineering and operational verification

**Complexity:** Medium (planning band; estimate after reconnaissance)

**Depends on:** [VOI-003](VOI-003-persist-confirmed-answers-and-late-transcript-enrichment.md), [WRK-003](WRK-003-route-overdue-work-using-clinic-hours-policy.md), [OPS-004](OPS-004-establish-an-isolated-synthetic-staging-environment.md), [GOV-001](GOV-001-define-and-implement-retention-access-and-deletion-policy.md)

**Blocks:** [QA-003](QA-003-measure-load-and-failure-recovery-at-the-planning-scale.md)
...
Prove one complete controlled real-call workflow. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Existing tests prove synthetic/HTTP/database behavior, not a real telephone call, durable callback and subsequent outreach suppression.

Use allowlisted consenting adult testers role-playing synthetic adults/minors/guardians; no real minor or patient data.
...
This ticket closes the gap through: prove one complete controlled real-call workflow. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Use allowlisted consenting adult testers role-playing synthetic adults/minors/guardians; no real minor or patient data. Limit initial run to one concurrent call and a small operator-approved spend cap. Record provider IDs in restricted evidence, never publish phone numbers/secrets. Controlled acceptance requires persisted answers, opt-out suppression and staff callback receipt after restart.
...
Ticket-specific boundary: Use allowlisted consenting adult testers role-playing synthetic adults/minors/guardians; no real minor or patient data. Limit initial run to one concurrent call and a small operator-approved spend cap. Record provider IDs in restricted evidence, never publish phone numbers/secrets. Controlled acceptance requires persisted answers, opt-out suppression and staff callback receipt after restart.

## 7. Facts, assumptions and unknowns

- **Fact:** Existing tests prove synthetic/HTTP/database behavior, not a real telephone call, durable callback and subsequent outreach suppression.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** VOI-003, WRK-003, OPS-004, GOV-001
...
Deliver prove one complete controlled real-call workflow and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Existing file: `tests/call-integrity.integration.test.cjs`](../../../tests/call-integrity.integration.test.cjs)
- [Existing file: `tests/fixtures/conversations/evaluation-fixtures.mjs`](../../../tests/fixtures/conversations/evaluation-fixtures.mjs)
...
- `docs/evidence/controlled-call.md`
- `tests/controlled-call.contract.test.cjs`
...
### Step 1: Verify deployment/provider/backup/eligibility gates and allowlist

**ACTION:** Verify deployment/provider/backup/eligibility gates and allowlist.

**WHY:** The required outcome is: no arbitrary destination or production patient can be dialed.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No arbitrary destination or production patient can be dialed. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Complete an approved synthetic script over a real telephone connection

**ACTION:** Complete an approved synthetic script over a real telephone connection.

**WHY:** The required outcome is: answers and transcript provenance persist with the expected occurrence.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Answers and transcript provenance persist with the expected occurrence. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Request callback and acknowledge it in the staff queue

**ACTION:** Request callback and acknowledge it in the staff queue.

**WHY:** The required outcome is: durable work exists independently of browser memory.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Durable work exists independently of browser memory. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Opt out and invoke next dispatch/retry attempt

**ACTION:** Opt out and invoke next dispatch/retry attempt.

**WHY:** The required outcome is: no new provider call is placed.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No new provider call is placed. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 5: Exercise disconnect, wrong person, duplicate webhook and lost provider response

**ACTION:** Exercise disconnect, wrong person, duplicate webhook and lost provider response.

**WHY:** The required outcome is: expected state and actual cost are recorded for every case.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Expected state and actual cost are recorded for every case. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: No arbitrary destination or production patient can be dialed.
- [ ] Step 2: Answers and transcript provenance persist with the expected occurrence.
- [ ] Step 3: Durable work exists independently of browser memory.
- [ ] Step 4: No new provider call is placed.
- [ ] Step 5: Expected state and actual cost are recorded for every case.

## 16. Error and failure semantics

If only simulator logs are available, this ticket is not complete. Compare provider evidence with canonical database records.
...
### TEST-QA-002-01: Real happy path

**Purpose:** Prove real happy path under the target contract.
...
**Action:** Exercise `Real happy path` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Telephone audio round trip, confirmed answers and staff-readable outcome persist after restart. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** If only simulator logs are available, this ticket is not complete. Compare provider evidence with canonical database records.

### TEST-QA-002-02: Callback durability

**Purpose:** Prove callback durability under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Callback durability` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Work is still present after application restart and staff login renewal. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** If only simulator logs are available, this ticket is not complete. Compare provider evidence with canonical database records.

### TEST-QA-002-03: Opt-out replay

**Purpose:** Prove opt-out replay under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Opt-out replay` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Subsequent scheduler/manual retry produces no provider side effect. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** If only simulator logs are available, this ticket is not complete. Compare provider evidence with canonical database records.

### TEST-QA-002-04: Failure evidence

**Purpose:** Prove failure evidence under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Failure evidence` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Unknown dispatch is reconciled without duplicate call and failed outcomes are not labeled successful. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Telephone audio round trip, confirmed answers and staff-readable outcome persist after restart.
- [ ] Work is still present after application restart and staff login renewal.
- [ ] Subsequent scheduler/manual retry produces no provider side effect.
- [ ] Unknown dispatch is reconciled without duplicate call and failed outcomes are not labeled successful.
...
Disable live dispatch and media; reconcile active calls, retain all evidence and return to synthetic-only operation.
...
Notify the implementation owner of QA-003 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
