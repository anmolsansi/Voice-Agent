# CBK-004: Complete staff-entered callback handling
...
Complete staff-entered callback handling. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Release1 queue primitives need standalone creation, identity verification and complete operational handling.

Authorized care staff may create a request on a caller behalf with server actor and source.
...
This ticket closes the gap through: complete staff-entered callback handling. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Authorized care staff may create a request on a caller behalf with server actor and source. Staff verifies association, assigns, acknowledges, documents contact attempts and resolves with approved reason codes. All mutations use revisions and audit; request text never becomes a staff identity. Unsuccessful contact remains actionable under policy.
...
Ticket-specific boundary: Authorized care staff may create a request on a caller behalf with server actor and source. Staff verifies association, assigns, acknowledges, documents contact attempts and resolves with approved reason codes. All mutations use revisions and audit; request text never becomes a staff identity. Unsuccessful contact remains actionable under policy.

## 7. Facts, assumptions and unknowns

- **Fact:** Release1 queue primitives need standalone creation, identity verification and complete operational handling.
...
Deliver complete staff-entered callback handling and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `tests/browser/callback-staff.spec.ts`
...
### Step 1: Add staff creation and source/provenance UI

**ACTION:** Add staff creation and source/provenance UI.

**WHY:** The required outcome is: entry can be distinguished from patient request.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Entry can be distinguished from patient request. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Connect assignment/acknowledgment and contact attempts

**ACTION:** Connect assignment/acknowledgment and contact attempts.

**WHY:** The required outcome is: ownership and status are independently visible.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Ownership and status are independently visible. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Add resolution/reopen actions with reasons

**ACTION:** Add resolution/reopen actions with reasons.

**WHY:** The required outcome is: unresolved contact cannot disappear as delivered.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Unresolved contact cannot disappear as delivered. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Test simultaneous staff handling

**ACTION:** Test simultaneous staff handling.

**WHY:** The required outcome is: conflicts require refresh rather than overwrite.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Conflicts require refresh rather than overwrite. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Entry can be distinguished from patient request.
- [ ] Step 2: Ownership and status are independently visible.
- [ ] Step 3: Unresolved contact cannot disappear as delivered.
- [ ] Step 4: Conflicts require refresh rather than overwrite.

## 16. Error and failure semantics

Check work revision and resolution reason before changing status rules.
...
### TEST-CBK-004-01: Forged actor

**Purpose:** Prove forged actor under the target contract.
...
**Action:** Exercise `Forged actor` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Backend ignores/rejects caller-supplied staff identity. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Check work revision and resolution reason before changing status rules.

### TEST-CBK-004-02: Concurrent resolution

**Purpose:** Prove concurrent resolution under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Concurrent resolution` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** One revision commits with audit. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Check work revision and resolution reason before changing status rules.

### TEST-CBK-004-03: Failed contact

**Purpose:** Prove failed contact under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Failed contact` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Work remains actionable and next step is recorded. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Backend ignores/rejects caller-supplied staff identity.
- [ ] One revision commits with audit.
- [ ] Work remains actionable and next step is recorded.
...
Disable unsafe actions while maintaining authenticated queue visibility and documented manual procedure.
