# AUTH-005: Add administrator MFA and audited recovery

**Status: Pending — Not completed**

**Priority:** P0 · **Area:** A — Truth and security · **Type:** Engineering and operational verification
...
**Blocks:** [ADM-001](ADM-001-complete-staff-administration-and-clinic-settings.md), [OPS-001](OPS-001-package-one-clinic-on-a-low-fixed-cost-vm.md)
...
Add administrator MFA and audited recovery. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Staff passwords and revocable sessions exist; no MFA enrollment or challenge is present in the current auth routes.

Require TOTP for admins before commercial activation.
...
This ticket closes the gap through: add administrator mfa and audited recovery. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Require TOTP for admins before commercial activation. Use a vetted implementation; encrypt enrollment secrets with an operator-managed key, hash one-use recovery codes, and record session authentication level. Password-only or pending-enrollment sessions may access setup/recovery endpoints only. Operator recovery revokes sessions and requires re-enrollment.
...
Ticket-specific boundary: Require TOTP for admins before commercial activation. Use a vetted implementation; encrypt enrollment secrets with an operator-managed key, hash one-use recovery codes, and record session authentication level. Password-only or pending-enrollment sessions may access setup/recovery endpoints only. Operator recovery revokes sessions and requires re-enrollment.

## 7. Facts, assumptions and unknowns

- **Fact:** Staff passwords and revocable sessions exist; no MFA enrollment or challenge is present in the current auth routes.
...
Deliver add administrator mfa and audited recovery and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Existing file: `src/modules/auth/routes.js`](../../../src/modules/auth/routes.js)
- [Existing file: `db/migrations/005_staff_identity.cjs`](../../../db/migrations/005_staff_identity.cjs)
- [Existing file: `components/staff-login-form.tsx`](../../../components/staff-login-form.tsx)
...
- `src/modules/auth/mfa.js`
- `tests/mfa.integration.test.cjs`
...
### Step 1: Add additive enrollment/recovery persistence and authentication-level fields

**ACTION:** Add additive enrollment/recovery persistence and authentication-level fields.

**WHY:** The required outcome is: no applied005 migration is edited and legacy admins enter enrollment-required state.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No applied005 migration is edited and legacy admins enter enrollment-required state. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Implement authenticated pending enrollment and confirm a valid TOTP before activation

**ACTION:** Implement authenticated pending enrollment and confirm a valid TOTP before activation.

**WHY:** The required outcome is: merely requesting a QR/setup secret does not mark MFA enabled.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Merely requesting a QR/setup secret does not mark MFA enabled. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Require password plus MFA challenge for admin session issuance

**ACTION:** Require password plus MFA challenge for admin session issuance.

**WHY:** The required outcome is: a password-only session cannot reach privileged routes.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** A password-only session cannot reach privileged routes. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Implement one-use recovery with throttling and mandatory audit

**ACTION:** Implement one-use recovery with throttling and mandatory audit.

**WHY:** The required outcome is: parallel recovery requests consume a code at most once.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Parallel recovery requests consume a code at most once. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 5: Add browser enrollment/challenge/error states and operator instructions

**ACTION:** Add browser enrollment/challenge/error states and operator instructions.

**WHY:** The required outcome is: an administrator can recover without an unaudited bypass.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** An administrator can recover without an unaudited bypass. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: No applied005 migration is edited and legacy admins enter enrollment-required state.
- [ ] Step 2: Merely requesting a QR/setup secret does not mark MFA enabled.
- [ ] Step 3: A password-only session cannot reach privileged routes.
- [ ] Step 4: Parallel recovery requests consume a code at most once.
- [ ] Step 5: An administrator can recover without an unaudited bypass.

## 16. Error and failure semantics

Check session auth-level enforcement on the backend, not only navigation redirects. Clock drift diagnostics must not reveal secrets.
...
### TEST-AUTH-005-01: Password-only admin access

**Purpose:** Prove password-only admin access under the target contract.
...
**Action:** Exercise `Password-only admin access` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Management and administrative call actions are denied until MFA completes. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Check session auth-level enforcement on the backend, not only navigation redirects. Clock drift diagnostics must not reveal secrets.

### TEST-AUTH-005-02: Recovery-code race

**Purpose:** Prove recovery-code race under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Recovery-code race` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Exactly one use succeeds; existing sessions are revoked and audit is durable. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Check session auth-level enforcement on the backend, not only navigation redirects. Clock drift diagnostics must not reveal secrets.

### TEST-AUTH-005-03: Invalid/replayed challenge

**Purpose:** Prove invalid/replayed challenge under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Invalid/replayed challenge` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** No privileged session is created; limits apply and secrets never appear in logs. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Check session auth-level enforcement on the backend, not only navigation redirects. Clock drift diagnostics must not reveal secrets.

### TEST-AUTH-005-04: Upgrade enrollment

**Purpose:** Prove upgrade enrollment under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Upgrade enrollment` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Existing admin has a documented secure enrollment path without losing the last-admin invariant. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Management and administrative call actions are denied until MFA completes.
- [ ] Exactly one use succeeds; existing sessions are revoked and audit is durable.
- [ ] No privileged session is created; limits apply and secrets never appear in logs.
- [ ] Existing admin has a documented secure enrollment path without losing the last-admin invariant.
...
Disable privileged operations if MFA fails after activation; do not globally bypass MFA to restore availability.
...
Notify the implementation owner of ADM-001, OPS-001 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
