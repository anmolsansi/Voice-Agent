# SCH-001: Model post-visit occurrences and immutable script versions
...
**Depends on:** [PAT-001](PAT-001-build-canonical-staff-patient-management.md), [CNS-001](CNS-001-persist-consent-and-communication-preference-history.md), [DEC-004](DEC-004-approve-clinic-hours-follow-up-and-safety-responsibilities.md)

**Blocks:** [SCH-002](SCH-002-implement-scheduling-windows-recurrence-and-bounded-retries.md)
...
Model post-visit occurrences and immutable script versions. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Schedules and attempts exist but there is no durable business occurrence binding a visit and approved script version.

Add checkin_occurrences(patient_id,schedule_id,visit_reference,script_version_id,due_at,state) and immutable approved script_versions.
...
This ticket closes the gap through: model post-visit occurrences and immutable script versions. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Add checkin_occurrences(patient_id,schedule_id,visit_reference,script_version_id,due_at,state) and immutable approved script_versions. Attempts retain lifecycle ownership and reference occurrence_id. Unique(schedule_id,due_at) prevents duplicate occurrences. Business states pending/completed/canceled/exhausted do not replace attempt states. Legacy attempts may remain explicitly legacy/unmapped until verified, without invented visit links.
...
Ticket-specific boundary: Add checkin_occurrences(patient_id,schedule_id,visit_reference,script_version_id,due_at,state) and immutable approved script_versions. Attempts retain lifecycle ownership and reference occurrence_id. Unique(schedule_id,due_at) prevents duplicate occurrences. Business states pending/completed/canceled/exhausted do not replace attempt states. Legacy attempts may remain explicitly legacy/unmapped until verified, without invented visit links.

## 7. Facts, assumptions and unknowns

- **Fact:** Schedules and attempts exist but there is no durable business occurrence binding a visit and approved script version.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** PAT-001, CNS-001, DEC-004
...
Deliver model post-visit occurrences and immutable script versions and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Existing file: `db/migrations/004_call_orchestration.cjs`](../../../db/migrations/004_call_orchestration.cjs)
...
- `src/modules/schedules/occurrences.js`
- `tests/occurrences.integration.test.cjs`
...
### Step 1: Audit existing schedule/attempt rows and classify legacy mappings

**ACTION:** Audit existing schedule/attempt rows and classify legacy mappings.

**WHY:** The required outcome is: no phone/name inference is used.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No phone/name inference is used. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Add versioned script/occurrence structures and optional legacy-safe attempt link

**ACTION:** Add versioned script/occurrence structures and optional legacy-safe attempt link.

**WHY:** The required outcome is: applied migrations stay intact.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Applied migrations stay intact. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Create occurrences idempotently from schedules

**ACTION:** Create occurrences idempotently from schedules.

**WHY:** The required outcome is: repeated generation yields one occurrence per due instant.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Repeated generation yields one occurrence per due instant. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Freeze script version when occurrence is created

**ACTION:** Freeze script version when occurrence is created.

**WHY:** The required outcome is: later script publication cannot rewrite historical meaning.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Later script publication cannot rewrite historical meaning. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 5: Record business completion separately from transport termination

**ACTION:** Record business completion separately from transport termination.

**WHY:** The required outcome is: a dropped call cannot masquerade as completed answers.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** A dropped call cannot masquerade as completed answers. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: No phone/name inference is used.
- [ ] Step 2: Applied migrations stay intact.
- [ ] Step 3: Repeated generation yields one occurrence per due instant.
- [ ] Step 4: Later script publication cannot rewrite historical meaning.
- [ ] Step 5: A dropped call cannot masquerade as completed answers.

## 16. Error and failure semantics

If reports count attempts as check-ins, inspect missing occurrence joins before changing metrics.
...
### TEST-SCH-001-01: Duplicate occurrence generation

**Purpose:** Prove duplicate occurrence generation under the target contract.
...
**Action:** Exercise `Duplicate occurrence generation` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** One occurrence and stable script version. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** If reports count attempts as check-ins, inspect missing occurrence joins before changing metrics.

### TEST-SCH-001-02: Script update

**Purpose:** Prove script update under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Script update` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Old occurrence retains original question/answer interpretation. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** If reports count attempts as check-ins, inspect missing occurrence joins before changing metrics.

### TEST-SCH-001-03: Legacy upgrade

**Purpose:** Prove legacy upgrade under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Legacy upgrade` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Unmapped legacy rows remain identifiable; migration neither deletes nor fabricates visits. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** If reports count attempts as check-ins, inspect missing occurrence joins before changing metrics.

### TEST-SCH-001-04: Transport completion without answers

**Purpose:** Prove transport completion without answers under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Transport completion without answers` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Occurrence does not become clinically/business complete. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] One occurrence and stable script version.
- [ ] Old occurrence retains original question/answer interpretation.
- [ ] Unmapped legacy rows remain identifiable; migration neither deletes nor fabricates visits.
- [ ] Occurrence does not become clinically/business complete.
...
Stop occurrence creation and retain all links/versions; old secure code may read legacy attempts but must not rewrite new business outcomes.
...
Notify the implementation owner of SCH-002 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
