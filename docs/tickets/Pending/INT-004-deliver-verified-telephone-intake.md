# INT-004: Deliver verified telephone intake
...
**Depends on:** [INT-002](INT-002-complete-patient-and-guardian-access-expiry-and-resume.md), [VOI-003](VOI-003-persist-confirmed-answers-and-late-transcript-enrichment.md)
...
Deliver verified telephone intake. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Post-visit voice infrastructure does not by itself implement telephone intake identity, draft or submission policy.

Telephone entry follows explicit verified respondent association and the shared intake schema.
...
This ticket closes the gap through: deliver verified telephone intake. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Telephone entry follows explicit verified respondent association and the shared intake schema. Caller ID is contact context only. Bind one conversation to one intake draft and preserve confirmed partial answers after disconnect. Approved readback/review or secure browser review is required before immutable submission; uncertainty routes to staffed work.
...
Ticket-specific boundary: Telephone entry follows explicit verified respondent association and the shared intake schema. Caller ID is contact context only. Bind one conversation to one intake draft and preserve confirmed partial answers after disconnect. Approved readback/review or secure browser review is required before immutable submission; uncertainty routes to staffed work.

## 7. Facts, assumptions and unknowns

- **Fact:** Post-visit voice infrastructure does not by itself implement telephone intake identity, draft or submission policy.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** INT-002, VOI-003
...
Deliver deliver verified telephone intake and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `tests/telephone-intake.integration.test.cjs`
...
### Step 1: Add purpose-specific routing and respondent verification

**ACTION:** Add purpose-specific routing and respondent verification.

**WHY:** The required outcome is: no intake data is disclosed to an unverified caller.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No intake data is disclosed to an unverified caller. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Adapt fixed intake script to shared validators

**ACTION:** Adapt fixed intake script to shared validators.

**WHY:** The required outcome is: voice-only path cannot bypass required fields.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Voice-only path cannot bypass required fields. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Persist confirmed answers with revision/provenance

**ACTION:** Persist confirmed answers with revision/provenance.

**WHY:** The required outcome is: disconnect does not lose committed answers.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Disconnect does not lose committed answers. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Implement review, consent and staff handoff

**ACTION:** Implement review, consent and staff handoff.

**WHY:** The required outcome is: submission has recorded respondent confirmation.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Submission has recorded respondent confirmation. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: No intake data is disclosed to an unverified caller.
- [ ] Step 2: Voice-only path cannot bypass required fields.
- [ ] Step 3: Disconnect does not lose committed answers.
- [ ] Step 4: Submission has recorded respondent confirmation.

## 16. Error and failure semantics

Inspect purpose association and verification state before prompt behavior.
...
### TEST-INT-004-01: Unverified caller

**Purpose:** Prove unverified caller under the target contract.
...
**Action:** Exercise `Unverified caller` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Generic guidance only, no patient details. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Inspect purpose association and verification state before prompt behavior.

### TEST-INT-004-02: Mid-call disconnect

**Purpose:** Prove mid-call disconnect under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Mid-call disconnect` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Confirmed draft resumes under verified access. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Inspect purpose association and verification state before prompt behavior.

### TEST-INT-004-03: Readback correction

**Purpose:** Prove readback correction under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Readback correction` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Corrected confirmed value appears in immutable snapshot. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Generic guidance only, no patient details.
- [ ] Confirmed draft resumes under verified access.
- [ ] Corrected confirmed value appears in immutable snapshot.
...
Disable telephone intake routing while keeping approved browser intake and existing call review.
