# AUD-001: Implement authoritative audit review and coverage
...
**Depends on:** [SEC-006](SEC-006-make-intake-drafts-submissions-and-review-atomic.md), [WRK-002](WRK-002-build-staff-queue-assignment-acknowledgment-and-resolution.md), [RPT-001](RPT-001-replace-fixed-reports-and-make-exports-safe.md)
...
Implement authoritative audit review and coverage. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

The audit-log API is a501 placeholder although foundation mutations already write authoritative audit events.

Admin-only audit read uses cursor pagination, bounded filters and immutable audit IDs.
...
This ticket closes the gap through: implement authoritative audit review and coverage. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Admin-only audit read uses cursor pagination, bounded filters and immutable audit IDs. Mandatory domain mutations and their audit commit atomically. Do not duplicate historical call-detail audit records into a competing live trail. Store actor/session reference, action, entity ID, request ID and safe change metadata; exclude secrets, transcript and patient payload.
...
Ticket-specific boundary: Admin-only audit read uses cursor pagination, bounded filters and immutable audit IDs. Mandatory domain mutations and their audit commit atomically. Do not duplicate historical call-detail audit records into a competing live trail. Store actor/session reference, action, entity ID, request ID and safe change metadata; exclude secrets, transcript and patient payload.

## 7. Facts, assumptions and unknowns

- **Fact:** The audit-log API is a501 placeholder although foundation mutations already write authoritative audit events.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** SEC-006, WRK-002, RPT-001
...
Deliver implement authoritative audit review and coverage and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `tests/audit-review.integration.test.cjs`
...
### Step 1: Inventory mutation-to-audit coverage

**ACTION:** Inventory mutation-to-audit coverage.

**WHY:** The required outcome is: missing actions are explicit and classified mandatory.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Missing actions are explicit and classified mandatory. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Implement authorized read API and audit screen

**ACTION:** Implement authorized read API and audit screen.

**WHY:** The required outcome is: only admin sees bounded metadata.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Only admin sees bounded metadata. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Fill missing atomic audit writes

**ACTION:** Fill missing atomic audit writes.

**WHY:** The required outcome is: failed audit inserts roll back associated mutations.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Failed audit inserts roll back associated mutations. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Document retention and export policy with GOV-001

**ACTION:** Document retention and export policy with GOV-001.

**WHY:** The required outcome is: audit retention does not silently expose deleted content.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Audit retention does not silently expose deleted content. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Missing actions are explicit and classified mandatory.
- [ ] Step 2: Only admin sees bounded metadata.
- [ ] Step 3: Failed audit inserts roll back associated mutations.
- [ ] Step 4: Audit retention does not silently expose deleted content.

## 16. Error and failure semantics

Trace the transaction client passed to domain and audit repositories; separate pool clients break atomicity.
...
### TEST-AUD-001-01: Audit insert failure

**Purpose:** Prove audit insert failure under the target contract.
...
**Action:** Exercise `Audit insert failure` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Mutation and audit both remain absent. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Trace the transaction client passed to domain and audit repositories; separate pool clients break atomicity.

### TEST-AUD-001-02: Pagination
...
**Expected assertions:** Equal timestamps neither duplicate nor skip IDs. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Trace the transaction client passed to domain and audit repositories; separate pool clients break atomicity.

### TEST-AUD-001-03: Sensitive-data scan

**Purpose:** Prove sensitive-data scan under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Sensitive-data scan` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Passwords, tokens and clinical content do not appear in stored audit metadata. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Mutation and audit both remain absent.
- [ ] Equal timestamps neither duplicate nor skip IDs.
- [ ] Passwords, tokens and clinical content do not appear in stored audit metadata.
...
Restrict audit UI if needed; never disable mandatory writes to make a mutation succeed.
