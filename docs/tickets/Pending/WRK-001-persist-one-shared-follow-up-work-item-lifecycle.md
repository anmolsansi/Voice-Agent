# WRK-001: Persist one shared follow-up work-item lifecycle
...
**Depends on:** [PAT-002](PAT-002-persist-guardian-relationships-and-enforce-authority.md), [DEC-004](DEC-004-approve-clinic-hours-follow-up-and-safety-responsibilities.md)

**Blocks:** [WRK-002](WRK-002-build-staff-queue-assignment-acknowledgment-and-resolution.md), [CBK-001](CBK-001-model-standalone-callback-requests-independently.md)
...
Persist one shared follow-up work-item lifecycle. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

The current escalation service transforms arrays and fixture state; there is no authoritative durable staff queue.

Propose work_items with source_type/source_id, optional patient/guardian/contact reference, verification state, priority, status, assignee, acknowledged_at/by, due_at, resolution and revision.
...
This ticket closes the gap through: persist one shared follow-up work-item lifecycle. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Propose work_items with source_type/source_id, optional patient/guardian/contact reference, verification state, priority, status, assignee, acknowledged_at/by, due_at, resolution and revision. States open→acknowledged→in_progress→resolved/canceled; assignment is independent. Reopen is explicit audited transition to open with reason. Unique source dedupe prevents repeated webhook-created work. Mutation+audit atomic.
...
Ticket-specific boundary: Propose work_items with source_type/source_id, optional patient/guardian/contact reference, verification state, priority, status, assignee, acknowledged_at/by, due_at, resolution and revision. States open→acknowledged→in_progress→resolved/canceled; assignment is independent. Reopen is explicit audited transition to open with reason. Unique source dedupe prevents repeated webhook-created work. Mutation+audit atomic.

## 7. Facts, assumptions and unknowns

- **Fact:** The current escalation service transforms arrays and fixture state; there is no authoritative durable staff queue.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** PAT-002, DEC-004
...
Deliver persist one shared follow-up work-item lifecycle and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `src/modules/work-items/service.js`
- `src/modules/work-items/routes.js`
- `tests/work-items.integration.test.cjs`
...
### Step 1: Map policy reasons/priorities to required fields and closure evidence

**ACTION:** Map policy reasons/priorities to required fields and closure evidence.

**WHY:** The required outcome is: no AI-generated arbitrary priority or missing ownership rule.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No AI-generated arbitrary priority or missing ownership rule. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Add work-item/history/unique-source persistence

**ACTION:** Add work-item/history/unique-source persistence.

**WHY:** The required outcome is: reload and restart cannot lose follow-up.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Reload and restart cannot lose follow-up. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Implement authenticated list/read/transition/assign service

**ACTION:** Implement authenticated list/read/transition/assign service.

**WHY:** The required outcome is: expected revision prevents stale overwrites.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Expected revision prevents stale overwrites. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Connect confirmed callback/safety outcomes to work creation transaction

**ACTION:** Connect confirmed callback/safety outcomes to work creation transaction.

**WHY:** The required outcome is: event replay cannot duplicate staff obligations.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Event replay cannot duplicate staff obligations. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: No AI-generated arbitrary priority or missing ownership rule.
- [ ] Step 2: Reload and restart cannot lose follow-up.
- [ ] Step 3: Expected revision prevents stale overwrites.
- [ ] Step 4: Event replay cannot duplicate staff obligations.

## 16. Error and failure semantics

Distinguish assignment from acknowledgment; assigning a user does not prove the user saw the request.
...
### TEST-WRK-001-01: Duplicate source event

**Purpose:** Prove duplicate source event under the target contract.
...
**Action:** Exercise `Duplicate source event` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Exactly one work item and initial audit. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Distinguish assignment from acknowledgment; assigning a user does not prove the user saw the request.

### TEST-WRK-001-02: Resolution without required note

**Purpose:** Prove resolution without required note under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Resolution without required note` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Rejected; status unchanged. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Distinguish assignment from acknowledgment; assigning a user does not prove the user saw the request.

### TEST-WRK-001-03: Concurrent assignment/status

**Purpose:** Prove concurrent assignment/status under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Concurrent assignment/status` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** One stale revision conflicts; final owner/outcome is consistent. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Distinguish assignment from acknowledgment; assigning a user does not prove the user saw the request.

### TEST-WRK-001-04: Restart

**Purpose:** Prove restart under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Restart` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Queue content and acknowledgment metadata persist. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Exactly one work item and initial audit.
- [ ] Rejected; status unchanged.
- [ ] One stale revision conflicts; final owner/outcome is consistent.
- [ ] Queue content and acknowledgment metadata persist.
...
Stop automated creation if necessary but retain existing work/history; provide authorized read/export for manual continuity.
...
Notify the implementation owner of WRK-002, CBK-001 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
