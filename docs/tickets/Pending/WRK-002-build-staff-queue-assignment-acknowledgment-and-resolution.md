# WRK-002: Build staff queue assignment acknowledgment and resolution
...
**Depends on:** [WRK-001](WRK-001-persist-one-shared-follow-up-work-item-lifecycle.md)

**Blocks:** [WRK-003](WRK-003-route-overdue-work-using-clinic-hours-policy.md), [UI-001](UI-001-replace-synthetic-operational-dashboards-with-persisted-views.md), [AUD-001](AUD-001-implement-authoritative-audit-review-and-coverage.md)
...
Build staff queue assignment acknowledgment and resolution. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

The current dashboard shows synthetic counters and has no persisted work-item handling screen.

Staff queue reads /api/work-items with stable cursor, status/priority/assignee filters.
...
This ticket closes the gap through: build staff queue assignment acknowledgment and resolution. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Staff queue reads /api/work-items with stable cursor, status/priority/assignee filters. Mutations send expectedRevision and only update UI after committed success. Show open/unassigned/overdue distinctly; safe role visibility complements backend enforcement. Resolution requires policy evidence and displays history.
...
Ticket-specific boundary: Staff queue reads /api/work-items with stable cursor, status/priority/assignee filters. Mutations send expectedRevision and only update UI after committed success. Show open/unassigned/overdue distinctly; safe role visibility complements backend enforcement. Resolution requires policy evidence and displays history.

## 7. Facts, assumptions and unknowns

- **Fact:** The current dashboard shows synthetic counters and has no persisted work-item handling screen.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** WRK-001
...
Deliver build staff queue assignment acknowledgment and resolution and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Existing file: `components/staff-shell.tsx`](../../../components/staff-shell.tsx)
- [Existing file: `components/staff-states.tsx`](../../../components/staff-states.tsx)
- [Existing file: `lib/staff-client.ts`](../../../lib/staff-client.ts)
- [Existing file: `app/dashboard/page.tsx`](../../../app/dashboard/page.tsx)

Proposed output paths (not present merely because this ticket exists):

- `app/dashboard/work/page.tsx`
- `components/work-item-panel.tsx`
- `tests/browser/work-queue.spec.ts`
...
### Step 1: Build paginated persisted queue and detail reads

**ACTION:** Build paginated persisted queue and detail reads.

**WHY:** The required outcome is: empty data is not replaced by fixture alerts.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Empty data is not replaced by fixture alerts. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Implement acknowledge/assign/reassign/start/resolve/cancel controls

**ACTION:** Implement acknowledge/assign/reassign/start/resolve/cancel controls.

**WHY:** The required outcome is: actions reflect allowed transitions and role permissions.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Actions reflect allowed transitions and role permissions. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Handle409 with reload and retained unsent notes

**ACTION:** Handle409 with reload and retained unsent notes.

**WHY:** The required outcome is: a stale user cannot unknowingly overwrite another staff member.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** A stale user cannot unknowingly overwrite another staff member. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Add accessible loading/error/retry and keyboard behavior

**ACTION:** Add accessible loading/error/retry and keyboard behavior.

**WHY:** The required outcome is: no action is inaccessible without a mouse or falsely successful.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No action is inaccessible without a mouse or falsely successful. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Empty data is not replaced by fixture alerts.
- [ ] Step 2: Actions reflect allowed transitions and role permissions.
- [ ] Step 3: A stale user cannot unknowingly overwrite another staff member.
- [ ] Step 4: No action is inaccessible without a mouse or falsely successful.

## 16. Error and failure semantics

If UI says resolved but database does not, remove optimistic success and inspect mutation response handling.
...
### TEST-WRK-002-01: Two staff browser race

**Purpose:** Prove two staff browser race under the target contract.
...
**Action:** Exercise `Two staff browser race` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Losing writer sees conflict; winning owner and notes remain intact. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** If UI says resolved but database does not, remove optimistic success and inspect mutation response handling.

### TEST-WRK-002-02: Backend failure

**Purpose:** Prove backend failure under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Backend failure` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Row status stays unchanged and user notes are retained. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** If UI says resolved but database does not, remove optimistic success and inspect mutation response handling.

### TEST-WRK-002-03: Pagination/filter refresh

**Purpose:** Prove pagination/filter refresh under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Pagination/filter refresh` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Same persisted work is discoverable beyond first page and after login. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** If UI says resolved but database does not, remove optimistic success and inspect mutation response handling.

### TEST-WRK-002-04: Role bypass

**Purpose:** Prove role bypass under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Role bypass` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Direct unauthorized mutation fails even if UI control is forged. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Losing writer sees conflict; winning owner and notes remain intact.
- [ ] Row status stays unchanged and user notes are retained.
- [ ] Same persisted work is discoverable beyond first page and after login.
- [ ] Direct unauthorized mutation fails even if UI control is forged.
...
Disable mutation controls and retain read-only queue/manual continuity; do not drop work records.
...
Notify the implementation owner of WRK-003, UI-001, AUD-001 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
