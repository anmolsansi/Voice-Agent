# PAT-003: Add previewed idempotent patient CSV import

**Status: Pending — Not completed**

**Priority:** P1 · **Area:** C — Patient and scheduling data · **Type:** Engineering

**Complexity:** Medium (planning band; estimate after reconnaissance)

**Depends on:** [PAT-001](PAT-001-build-canonical-staff-patient-management.md), [PAT-002](PAT-002-persist-guardian-relationships-and-enforce-authority.md)
...
Add previewed idempotent patient CSV import. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

No operational patient CSV preview/apply workflow exists.

Admin POST /api/patient-imports/preview accepts UTF-8 CSV <=5MiB and10000 rows; returns durable batchId, counts and row errors without creating patients.
...
This ticket closes the gap through: add previewed idempotent patient csv import. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Admin POST /api/patient-imports/preview accepts UTF-8 CSV <=5MiB and10000 rows; returns durable batchId, counts and row errors without creating patients. POST /api/patient-imports/:id/apply requires matching batch revision and idempotency key. Initial apply is all-or-nothing for valid approved batches; ambiguous identity blocks apply. Use explicit external IDs, not phone/name deduplication. Preview expires after24h and is governed by retention.
...
Ticket-specific boundary: Admin POST /api/patient-imports/preview accepts UTF-8 CSV <=5MiB and10000 rows; returns durable batchId, counts and row errors without creating patients. POST /api/patient-imports/:id/apply requires matching batch revision and idempotency key. Initial apply is all-or-nothing for valid approved batches; ambiguous identity blocks apply. Use explicit external IDs, not phone/name deduplication. Preview expires after24h and is governed by retention.

## 7. Facts, assumptions and unknowns

- **Fact:** No operational patient CSV preview/apply workflow exists.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** PAT-001, PAT-002
...
Deliver add previewed idempotent patient csv import and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `src/modules/patients/import-service.js`
- `app/dashboard/patients/import/page.tsx`
- `tests/patient-import.integration.test.cjs`
...
### Step 1: Publish versioned columns, normalization and sample rows

**ACTION:** Publish versioned columns, normalization and sample rows.

**WHY:** The required outcome is: every field maps to patient/contact/guardian scope.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Every field maps to patient/contact/guardian scope. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Bound streaming bytes/rows and stage a preview

**ACTION:** Bound streaming bytes/rows and stage a preview.

**WHY:** The required outcome is: invalid or oversized input makes no patient writes.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Invalid or oversized input makes no patient writes. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Classify create/update/conflict by explicit IDs and authority

**ACTION:** Classify create/update/conflict by explicit IDs and authority.

**WHY:** The required outcome is: ambiguity is presented for staff resolution.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Ambiguity is presented for staff resolution. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Apply the approved immutable preview in a transaction with receipt/audit

**ACTION:** Apply the approved immutable preview in a transaction with receipt/audit.

**WHY:** The required outcome is: retry returns the same batch result and counts.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Retry returns the same batch result and counts. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 5: Build preview/error/download/apply states

**ACTION:** Build preview/error/download/apply states.

**WHY:** The required outcome is: failed apply never shows partial success or loses diagnostics.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Failed apply never shows partial success or loses diagnostics. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Every field maps to patient/contact/guardian scope.
- [ ] Step 2: Invalid or oversized input makes no patient writes.
- [ ] Step 3: Ambiguity is presented for staff resolution.
- [ ] Step 4: Retry returns the same batch result and counts.
- [ ] Step 5: Failed apply never shows partial success or loses diagnostics.

## 16. Error and failure semantics

Compare preview hash/revision to apply input. Do not rerun parsing against changed input under the same approved batch ID.
...
### TEST-PAT-003-01: Invalid row/ambiguous ID

**Purpose:** Prove invalid row/ambiguous ID under the target contract.
...
**Action:** Exercise `Invalid row/ambiguous ID` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Apply is blocked and patient counts remain unchanged. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Compare preview hash/revision to apply input. Do not rerun parsing against changed input under the same approved batch ID.

### TEST-PAT-003-02: Repeated apply

**Purpose:** Prove repeated apply under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Repeated apply` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Same receipt and IDs; zero duplicate patients. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Compare preview hash/revision to apply input. Do not rerun parsing against changed input under the same approved batch ID.

### TEST-PAT-003-03: Tampered/expired preview

**Purpose:** Prove tampered/expired preview under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Tampered/expired preview` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** 409/410 before write; changes require a fresh preview. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Compare preview hash/revision to apply input. Do not rerun parsing against changed input under the same approved batch ID.

### TEST-PAT-003-04: Atomic failure

**Purpose:** Prove atomic failure under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Atomic failure` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Injected middle-row/audit failure leaves no partial batch. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Apply is blocked and patient counts remain unchanged.
- [ ] Same receipt and IDs; zero duplicate patients.
- [ ] 409/410 before write; changes require a fresh preview.
- [ ] Injected middle-row/audit failure leaves no partial batch.
...
Disable import UI/API; preserve audit and imported identity history. Undo only through explicit reviewed corrections, not bulk deletion.
