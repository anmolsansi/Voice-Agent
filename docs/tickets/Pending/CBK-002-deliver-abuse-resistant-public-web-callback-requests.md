# CBK-002: Deliver abuse-resistant public web callback requests
...
**Depends on:** [CBK-001](CBK-001-model-standalone-callback-requests-independently.md)
...
Deliver abuse-resistant public web callback requests. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

A standalone public callback request form is not an accepted product workflow.

Public form collects only approved minimum contact, callback window and non-emergency reason categories.
...
This ticket closes the gap through: deliver abuse-resistant public web callback requests. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Public form collects only approved minimum contact, callback window and non-emergency reason categories. It gives clinic-hours expectations and emergency guidance approved by DEC-004. Bound input, rate-limit persistently, prevent enumeration and support accessible retries. No arbitrary medical uploads or public request lookup by predictable ID.
...
Ticket-specific boundary: Public form collects only approved minimum contact, callback window and non-emergency reason categories. It gives clinic-hours expectations and emergency guidance approved by DEC-004. Bound input, rate-limit persistently, prevent enumeration and support accessible retries. No arbitrary medical uploads or public request lookup by predictable ID.

## 7. Facts, assumptions and unknowns

- **Fact:** A standalone public callback request form is not an accepted product workflow.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** CBK-001
...
Deliver deliver abuse-resistant public web callback requests and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `tests/browser/callback-request.spec.ts`
...
### Step 1: Implement minimal form and approved disclosures

**ACTION:** Implement minimal form and approved disclosures.

**WHY:** The required outcome is: expectations match staffed service hours.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Expectations match staffed service hours. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Add bounded validated request API and abuse controls

**ACTION:** Add bounded validated request API and abuse controls.

**WHY:** The required outcome is: floods cannot exhaust work/database capacity.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Floods cannot exhaust work/database capacity. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Return safe durable receipt after commit

**ACTION:** Return safe durable receipt after commit.

**WHY:** The required outcome is: uI never promises receipt before persistence.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** UI never promises receipt before persistence. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Support idempotent retry and accessible errors

**ACTION:** Support idempotent retry and accessible errors.

**WHY:** The required outcome is: timeout retry does not duplicate work.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Timeout retry does not duplicate work. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Expectations match staffed service hours.
- [ ] Step 2: Floods cannot exhaust work/database capacity.
- [ ] Step 3: UI never promises receipt before persistence.
- [ ] Step 4: Timeout retry does not duplicate work.

## 16. Error and failure semantics

Compare durable request receipt with response timeout before asking the patient to resubmit.
...
### TEST-CBK-002-01: Repeated submission

**Purpose:** Prove repeated submission under the target contract.
...
**Action:** Exercise `Repeated submission` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** One request on retry with same key. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Compare durable request receipt with response timeout before asking the patient to resubmit.

### TEST-CBK-002-02: Oversize/flood

**Purpose:** Prove oversize/flood under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Oversize/flood` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Safe413/429 without creating partial work. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Compare durable request receipt with response timeout before asking the patient to resubmit.

### TEST-CBK-002-03: Enumeration

**Purpose:** Prove enumeration under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Enumeration` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Guessed IDs reveal no contact or patient data. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] One request on retry with same key.
- [ ] Safe413/429 without creating partial work.
- [ ] Guessed IDs reveal no contact or patient data.
...
Disable public entry and display approved alternative clinic contact information.
