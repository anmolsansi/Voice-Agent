# OPS-002: Prove encrypted backup and point-in-time recovery
...
**Depends on:** [OPS-001](OPS-001-package-one-clinic-on-a-low-fixed-cost-vm.md), [GOV-001](GOV-001-define-and-implement-retention-access-and-deletion-policy.md)
...
Prove encrypted backup and point-in-time recovery. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

No measured restore or recovery evidence exists for a deployed clinic instance.

Use encrypted off-host PostgreSQL base backups and WAL archiving with least-privilege storage credentials and tested retention.
...
This ticket closes the gap through: prove encrypted backup and point-in-time recovery. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Use encrypted off-host PostgreSQL base backups and WAL archiving with least-privilege storage credentials and tested retention. Provisional targets RPO15minutes/RTO4hours require measured proof and clinic approval. Back up required configuration and encryption-key recovery separately. Restores happen in an isolated network; restored workers cannot call patients until reconciled.
...
Ticket-specific boundary: Use encrypted off-host PostgreSQL base backups and WAL archiving with least-privilege storage credentials and tested retention. Provisional targets RPO15minutes/RTO4hours require measured proof and clinic approval. Back up required configuration and encryption-key recovery separately. Restores happen in an isolated network; restored workers cannot call patients until reconciled.

## 7. Facts, assumptions and unknowns

- **Fact:** No measured restore or recovery evidence exists for a deployed clinic instance.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** OPS-001, GOV-001
...
Deliver prove encrypted backup and point-in-time recovery and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Directory: `db`](../../../db)
- [Existing file: `docs/database-migrations.md`](../../../docs/database-migrations.md)
...
### Step 1: Choose supported backup tooling and independent storage

**ACTION:** Choose supported backup tooling and independent storage.

**WHY:** The required outcome is: a VM disk loss cannot destroy every recovery copy.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** A VM disk loss cannot destroy every recovery copy. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Automate backup/WAL health and retention

**ACTION:** Automate backup/WAL health and retention.

**WHY:** The required outcome is: missing archive intervals alert before RPO breach.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Missing archive intervals alert before RPO breach. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Restore to a chosen timestamp on a fresh isolated host

**ACTION:** Restore to a chosen timestamp on a fresh isolated host.

**WHY:** The required outcome is: deleted marker rows recover to the expected instant.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Deleted marker rows recover to the expected instant. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Document reconciliation, secret recovery and measured timings

**ACTION:** Document reconciliation, secret recovery and measured timings.

**WHY:** The required outcome is: operators can restore without the original VM.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Operators can restore without the original VM. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: A VM disk loss cannot destroy every recovery copy.
- [ ] Step 2: Missing archive intervals alert before RPO breach.
- [ ] Step 3: Deleted marker rows recover to the expected instant.
- [ ] Step 4: Operators can restore without the original VM.

## 16. Error and failure semantics

Inspect WAL continuity, object permissions and key availability; a successful backup command alone is not proof of restorable data.
...
### TEST-OPS-002-01: VM-loss rehearsal

**Purpose:** Prove vM-loss rehearsal under the target contract.
...
**Action:** Exercise `VM-loss rehearsal` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Recover database and configuration using off-host artifacts only. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Inspect WAL continuity, object permissions and key availability; a successful backup command alone is not proof of restorable data.

### TEST-OPS-002-02: Point-in-time restore

**Purpose:** Prove point-in-time restore under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Point-in-time restore` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Before/after marker records match the selected timestamp. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Inspect WAL continuity, object permissions and key availability; a successful backup command alone is not proof of restorable data.

### TEST-OPS-002-03: Restore isolation

**Purpose:** Prove restore isolation under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Restore isolation` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** No outbound provider call or staff notification is sent during restore. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Recover database and configuration using off-host artifacts only.
- [ ] Before/after marker records match the selected timestamp.
- [ ] No outbound provider call or staff notification is sent during restore.
...
Retain verified backup generations; pause deployment if no tested recovery point exists.
