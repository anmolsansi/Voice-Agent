# PAT-002: Persist guardian relationships and enforce authority
...
**Depends on:** [PAT-001](PAT-001-build-canonical-staff-patient-management.md), [DEC-003](DEC-003-approve-adult-minor-and-guardian-authority-rules.md)

**Blocks:** [PAT-003](PAT-003-add-previewed-idempotent-patient-csv-import.md), [CNS-001](CNS-001-persist-consent-and-communication-preference-history.md), [VOI-002](VOI-002-implement-approved-fixed-script-conversation-behavior.md), [WRK-001](WRK-001-persist-one-shared-follow-up-work-item-lifecycle.md), [INT-001](INT-001-define-one-versioned-intake-contract-for-both-voice-channels.md)
...
Persist guardian relationships and enforce authority. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

No durable guardian relationship exists in the current patient schema.

Add guardian identities and patient_guardians relationships with authority scopes, verification evidence reference, valid_from/expires_at/revoked_at and trusted verifier.
...
This ticket closes the gap through: persist guardian relationships and enforce authority. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Add guardian identities and patient_guardians relationships with authority scopes, verification evidence reference, valid_from/expires_at/revoked_at and trusted verifier. Staff manage relationships through /api/patients/:id/guardians. Store minimum evidence references, not unnecessary document copies. Every patient-capability and dispatch check reads current authority.
...
Ticket-specific boundary: Add guardian identities and patient_guardians relationships with authority scopes, verification evidence reference, valid_from/expires_at/revoked_at and trusted verifier. Staff manage relationships through /api/patients/:id/guardians. Store minimum evidence references, not unnecessary document copies. Every patient-capability and dispatch check reads current authority.

## 7. Facts, assumptions and unknowns

- **Fact:** No durable guardian relationship exists in the current patient schema.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** PAT-001, DEC-003
...
Deliver persist guardian relationships and enforce authority and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `src/modules/patients/guardians.js`
- `tests/guardian-access.integration.test.cjs`
...
### Step 1: Translate approved policy into relationship states and required evidence fields

**ACTION:** Translate approved policy into relationship states and required evidence fields.

**WHY:** The required outcome is: no hard-coded jurisdictional age rule is invented.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No hard-coded jurisdictional age rule is invented. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Add constraints and audited staff create/revoke operations

**ACTION:** Add constraints and audited staff create/revoke operations.

**WHY:** The required outcome is: relationship history remains after revocation.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Relationship history remains after revocation. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Centralize current-authority lookup for intake and outreach

**ACTION:** Centralize current-authority lookup for intake and outreach.

**WHY:** The required outcome is: all channels apply the same scopes and expiry.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** All channels apply the same scopes and expiry. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Display respondent identity separately from patient identity

**ACTION:** Display respondent identity separately from patient identity.

**WHY:** The required outcome is: answers and consent name the actual actor.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Answers and consent name the actual actor. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 5: Handle ambiguous/disputed links as blocked work for staff

**ACTION:** Handle ambiguous/disputed links as blocked work for staff.

**WHY:** The required outcome is: no automatic match by phone or surname.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No automatic match by phone or surname. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: No hard-coded jurisdictional age rule is invented.
- [ ] Step 2: Relationship history remains after revocation.
- [ ] Step 3: All channels apply the same scopes and expiry.
- [ ] Step 4: Answers and consent name the actual actor.
- [ ] Step 5: No automatic match by phone or surname.

## 16. Error and failure semantics

Check relationship validity at use time; a valid invitation issued before revocation must not freeze permission forever.
...
### TEST-PAT-002-01: Guardian isolation

**Purpose:** Prove guardian isolation under the target contract.
...
**Action:** Exercise `Guardian isolation` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** GuardianA authorized for patientA cannot access patientB. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Check relationship validity at use time; a valid invitation issued before revocation must not freeze permission forever.

### TEST-PAT-002-02: Revocation

**Purpose:** Prove revocation under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Revocation` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Existing browser/telephone capability loses authority for subsequent protected actions. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Check relationship validity at use time; a valid invitation issued before revocation must not freeze permission forever.

### TEST-PAT-002-03: Multiple/shared contacts

**Purpose:** Prove multiple/shared contacts under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Multiple/shared contacts` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Relationships remain explicit without merging guardians or patients. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Check relationship validity at use time; a valid invitation issued before revocation must not freeze permission forever.

### TEST-PAT-002-04: Expired authority

**Purpose:** Prove expired authority under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Expired authority` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Dispatch and access deny with a safe reason and no domain side effect. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] GuardianA authorized for patientA cannot access patientB.
- [ ] Existing browser/telephone capability loses authority for subsequent protected actions.
- [ ] Relationships remain explicit without merging guardians or patients.
- [ ] Dispatch and access deny with a safe reason and no domain side effect.
...
Pause guardian channels while retaining relationship/revocation history. Do not replace checks with a patient-supplied guardian flag.
...
Notify the implementation owner of PAT-003, CNS-001, VOI-002, WRK-001, INT-001 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
