# DOC-001: Reconcile current status and foundation evidence

**Status: Pending — Not completed**

**Priority:** P1 · **Area:** A — Truth and security · **Type:** Evidence and acceptance

**Complexity:** Medium (planning band; estimate after reconnaissance)

**Depends on:** No new-ticket dependency; use the merged foundation baseline.

**Blocks:** [SEC-001](SEC-001-resolve-dependency-vulnerabilities-with-regression-proof.md), [DEC-001](DEC-001-agree-a-clinic-buyer-scope-and-acceptance-contract.md), [QA-004](QA-004-maintain-requirement-to-evidence-acceptance-matrix.md)
...
Reconcile current status and foundation evidence. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

PR34 is merged and main CI passed, but foundation evidence still describes a draft/unmerged PR and historical product documents propose superseded tenancy and role designs.
...
This ticket closes the gap through: reconcile current status and foundation evidence. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Preserve historical evidence. Current status must distinguish implementation, verification, acceptance and deployment. The ten existing foundation packets retain their IDs and remain Pending until their own remaining review/operational criteria are resolved. A merge with no recorded GitHub reviews is not evidence that no review happened elsewhere.
...
Ticket-specific boundary: Preserve historical evidence. Current status must distinguish implementation, verification, acceptance and deployment. The ten existing foundation packets retain their IDs and remain Pending until their own remaining review/operational criteria are resolved. A merge with no recorded GitHub reviews is not evidence that no review happened elsewhere.

## 7. Facts, assumptions and unknowns

- **Fact:** PR34 is merged and main CI passed, but foundation evidence still describes a draft/unmerged PR and historical product documents propose superseded tenancy and role designs.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** Current source, foundation evidence and remote commit verification.
...
Deliver reconcile current status and foundation evidence and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Existing file: `docs/FOUNDATION_PROGRESS.md`](../../../docs/FOUNDATION_PROGRESS.md)
- [Existing file: `docs/FOUNDATION_EVIDENCE.md`](../../../docs/FOUNDATION_EVIDENCE.md)
- [Existing file: `docs/repo_context.md`](../../../docs/repo_context.md)
- [Existing file: `docs/product-guide/03-what-is-missing.md`](../../../docs/product-guide/03-what-is-missing.md)
- [Existing file: `docs/tickets/Pending/FOUND-001.md`](../../../docs/tickets/Pending/FOUND-001.md)
...
### Step 1: Compare current main, PR34, issue33 and each existing foundation packet

**ACTION:** Compare current main, PR34, issue33 and each existing foundation packet.

**WHY:** The required outcome is: each claim has a commit/run/operator evidence source.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Each claim has a commit/run/operator evidence source. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Add dated supersession notes to conflicting shared-token, shared-tenancy and builder descriptions

**ACTION:** Add dated supersession notes to conflicting shared-token, shared-tenancy and builder descriptions.

**WHY:** The required outcome is: historical wording cannot be mistaken for current instructions.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Historical wording cannot be mistaken for current instructions. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Evaluate every old ticket definition of done separately

**ACTION:** Evaluate every old ticket definition of done separately.

**WHY:** The required outcome is: missing acceptance is named without reimplementing proven code.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Missing acceptance is named without reimplementing proven code. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Update links and tracking only after the status evidence is reconciled

**ACTION:** Update links and tracking only after the status evidence is reconciled.

**WHY:** The required outcome is: one authoritative current status exists; no ticket is moved solely because its code merged.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** One authoritative current status exists; no ticket is moved solely because its code merged. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Each claim has a commit/run/operator evidence source.
- [ ] Step 2: Historical wording cannot be mistaken for current instructions.
- [ ] Step 3: Missing acceptance is named without reimplementing proven code.
- [ ] Step 4: One authoritative current status exists; no ticket is moved solely because its code merged.

## 16. Error and failure semantics

If two documents disagree, compare their dates and source commits; do not resolve disagreement by deleting evidence.
...
### TEST-DOC-001-01: Stale claim search

**Purpose:** Prove stale claim search under the target contract.
...
**Action:** Exercise `Stale claim search` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** No current section describes PR34 as open/draft or shared tokens as supported. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** If two documents disagree, compare their dates and source commits; do not resolve disagreement by deleting evidence.

### TEST-DOC-001-02: Status evidence audit

**Purpose:** Prove status evidence audit under the target contract.

**Level:** Artifact/approval review and observed acceptance exercise

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Status evidence audit` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Every Completed classification has its ticket-specific proof; unknown review/deployment fields stay unknown. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** If two documents disagree, compare their dates and source commits; do not resolve disagreement by deleting evidence.

### TEST-DOC-001-03: Preservation check

**Purpose:** Prove preservation check under the target contract.

**Level:** Artifact/approval review and observed acceptance exercise

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Preservation check` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Old IDs, original migration names and historical test counts remain traceable. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] No current section describes PR34 as open/draft or shared tokens as supported.
- [ ] Every Completed classification has its ticket-specific proof; unknown review/deployment fields stay unknown.
- [ ] Old IDs, original migration names and historical test counts remain traceable.
...
Revert erroneous documentation statements only; retain original source links and audit history.
...
Notify the implementation owner of SEC-001, DEC-001, QA-004 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
