# INT-006: Complete review, amendments and reproducible PDFs
...
**Depends on:** [INT-005](INT-005-resolve-cross-channel-intake-conflicts-and-recovery.md)

**Blocks:** [REL-002](REL-002-accept-complete-browser-and-telephone-intake-release.md)
...
Complete review, amendments and reproducible PDFs. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Existing review/PDF behavior is a baseline, not proof of versioned clinical amendments and cross-channel submissions.
...
This ticket closes the gap through: complete review, amendments and reproducible pdfs. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Submitted snapshots remain immutable. Staff review records actor/time and disposition; corrections create linked amendments with reason and version. PDF identifies patient/respondent, schema, submission/amendment version and review status. Generate only from the authorized stored snapshot, with no live-draft substitution and safe filenames/cache headers.
...
Ticket-specific boundary: Submitted snapshots remain immutable. Staff review records actor/time and disposition; corrections create linked amendments with reason and version. PDF identifies patient/respondent, schema, submission/amendment version and review status. Generate only from the authorized stored snapshot, with no live-draft substitution and safe filenames/cache headers.

## 7. Facts, assumptions and unknowns

- **Fact:** Existing review/PDF behavior is a baseline, not proof of versioned clinical amendments and cross-channel submissions.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** INT-005
...
Deliver complete review, amendments and reproducible pdfs and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `tests/intake-amendments.integration.test.cjs`
...
### Step 1: Define review/amendment permissions and immutable lineage

**ACTION:** Define review/amendment permissions and immutable lineage.

**WHY:** The required outcome is: original patient answers remain traceable.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Original patient answers remain traceable. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Implement review and correction workflows

**ACTION:** Implement review and correction workflows.

**WHY:** The required outcome is: staff cannot silently replace submitted content.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Staff cannot silently replace submitted content. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Render deterministic snapshot-specific PDFs

**ACTION:** Render deterministic snapshot-specific PDFs.

**WHY:** The required outcome is: download corresponds to the chosen version.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Download corresponds to the chosen version. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Verify multilingual text and pagination on supported templates

**ACTION:** Verify multilingual text and pagination on supported templates.

**WHY:** The required outcome is: no missing glyphs or clipped clinically relevant fields.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No missing glyphs or clipped clinically relevant fields. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Original patient answers remain traceable.
- [ ] Step 2: Staff cannot silently replace submitted content.
- [ ] Step 3: Download corresponds to the chosen version.
- [ ] Step 4: No missing glyphs or clipped clinically relevant fields.

## 16. Error and failure semantics

Compare PDF source snapshot ID before investigating rendering; correct layout cannot fix wrong source data.
...
### TEST-INT-006-01: Amendment

**Purpose:** Prove amendment under the target contract.
...
**Action:** Exercise `Amendment` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Original snapshot unchanged and correction linked with actor/reason. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Compare PDF source snapshot ID before investigating rendering; correct layout cannot fix wrong source data.

### TEST-INT-006-02: PDF version

**Purpose:** Prove pDF version under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `PDF version` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Prior and amended PDFs correspond to their own snapshots. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Compare PDF source snapshot ID before investigating rendering; correct layout cannot fix wrong source data.

### TEST-INT-006-03: Authorization

**Purpose:** Prove authorization under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Authorization` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Anonymous/disabled sessions cannot download. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Compare PDF source snapshot ID before investigating rendering; correct layout cannot fix wrong source data.

### TEST-INT-006-04: Layout fixtures

**Purpose:** Prove layout fixtures under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Layout fixtures` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Long answers and selected language render without clipping. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Original snapshot unchanged and correction linked with actor/reason.
- [ ] Prior and amended PDFs correspond to their own snapshots.
- [ ] Anonymous/disabled sessions cannot download.
- [ ] Long answers and selected language render without clipping.
...
Disable amendment writes or PDF delivery while retaining immutable review records.
...
Notify the implementation owner of REL-002 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
