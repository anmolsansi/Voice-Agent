# WRK-003: Route overdue work using clinic-hours policy

**Status: Pending — Not completed**

**Priority:** P0 · **Area:** E — Staff operations · **Type:** Engineering

**Complexity:** Medium (planning band; estimate after reconnaissance)

**Depends on:** [WRK-002](WRK-002-build-staff-queue-assignment-acknowledgment-and-resolution.md), [DEC-004](DEC-004-approve-clinic-hours-follow-up-and-safety-responsibilities.md)

**Blocks:** [QA-002](QA-002-prove-one-complete-controlled-real-call-workflow.md)
...
Route overdue work using clinic-hours policy. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

There is no durable notification delivery/acknowledgment mechanism or clinic-calendar deadline engine.

Calculate deadlines in clinic working time with IANA timezone and holiday calendar.
...
This ticket closes the gap through: route overdue work using clinic-hours policy. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Calculate deadlines in clinic working time with IANA timezone and holiday calendar. Persist delivery attempts with retry status and idempotency identity separate from work state. External notification contains opaque work ID/link and severity only. Delivery is never acknowledgment; missing staff acknowledgment triggers approved backup-owner route. Email/SMS channel is configured only if DEC-004/005 approves it; durable in-app queue always exists.
...
Ticket-specific boundary: Calculate deadlines in clinic working time with IANA timezone and holiday calendar. Persist delivery attempts with retry status and idempotency identity separate from work state. External notification contains opaque work ID/link and severity only. Delivery is never acknowledgment; missing staff acknowledgment triggers approved backup-owner route. Email/SMS channel is configured only if DEC-004/005 approves it; durable in-app queue always exists.

## 7. Facts, assumptions and unknowns

- **Fact:** There is no durable notification delivery/acknowledgment mechanism or clinic-calendar deadline engine.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** WRK-002, DEC-004
...
Deliver route overdue work using clinic-hours policy and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Existing file: `src/services/escalations/escalation-service.mjs`](../../../src/services/escalations/escalation-service.mjs)

Proposed output paths (not present merely because this ticket exists):

- `src/jobs/work-notifications.js`
- `tests/work-deadlines.integration.test.cjs`
...
### Step 1: Implement working-time deadline calculation from approved settings

**ACTION:** Implement working-time deadline calculation from approved settings.

**WHY:** The required outcome is: closure periods do not silently consume a business-hours promise.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Closure periods do not silently consume a business-hours promise. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Create durable reminder/escalation jobs with unique work/revision/kind key

**ACTION:** Create durable reminder/escalation jobs with unique work/revision/kind key.

**WHY:** The required outcome is: repeated sweeps do not flood notifications.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Repeated sweeps do not flood notifications. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Deliver outside database transactions with bounded retries

**ACTION:** Deliver outside database transactions with bounded retries.

**WHY:** The required outcome is: delivery outage never deletes work or changes acknowledged_at.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Delivery outage never deletes work or changes acknowledged_at. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Expose overdue delivery and ownership gaps to operators

**ACTION:** Expose overdue delivery and ownership gaps to operators.

**WHY:** The required outcome is: unreachable staff is a visible service failure.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Unreachable staff is a visible service failure. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Closure periods do not silently consume a business-hours promise.
- [ ] Step 2: Repeated sweeps do not flood notifications.
- [ ] Step 3: Delivery outage never deletes work or changes acknowledged_at.
- [ ] Step 4: Unreachable staff is a visible service failure.

## 16. Error and failure semantics

Investigate calendar policy separately from notification delivery. Do not mark work acknowledged to silence an alert.
...
### TEST-WRK-003-01: Weekend/DST/holiday deadline

**Purpose:** Prove weekend/DST/holiday deadline under the target contract.
...
**Action:** Exercise `Weekend/DST/holiday deadline` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Expected next staffed deadline exactly matches policy fixture. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Investigate calendar policy separately from notification delivery. Do not mark work acknowledged to silence an alert.

### TEST-WRK-003-02: Notification timeout/retry

**Purpose:** Prove notification timeout/retry under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Notification timeout/retry` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** One logical reminder with recorded attempts; work remains open. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Investigate calendar policy separately from notification delivery. Do not mark work acknowledged to silence an alert.

### TEST-WRK-003-03: Sent versus acknowledged

**Purpose:** Prove sent versus acknowledged under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Sent versus acknowledged` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Delivery success leaves acknowledgment empty until staff acts. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Investigate calendar policy separately from notification delivery. Do not mark work acknowledged to silence an alert.

### TEST-WRK-003-04: No primary staff

**Purpose:** Prove no primary staff under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `No primary staff` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Approved backup route executes and overdue item remains visible. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Expected next staffed deadline exactly matches policy fixture.
- [ ] One logical reminder with recorded attempts; work remains open.
- [ ] Delivery success leaves acknowledgment empty until staff acts.
- [ ] Approved backup route executes and overdue item remains visible.
...
Pause external reminders if faulty and retain overdue in-app queue/manual contact process; never erase deadlines.
...
Notify the implementation owner of QA-002 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
