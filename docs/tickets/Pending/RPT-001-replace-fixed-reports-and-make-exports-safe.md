# RPT-001: Replace fixed reports and make exports safe
...
**Depends on:** [UI-001](UI-001-replace-synthetic-operational-dashboards-with-persisted-views.md)

**Blocks:** [AUD-001](AUD-001-implement-authoritative-audit-review-and-coverage.md), [REL-001](REL-001-accept-the-complete-post-visit-check-in-release.md), [CBK-005](CBK-005-add-callback-reporting-and-recovery-evidence.md)
...
Replace fixed reports and make exports safe. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Reports use fixed synthetic records/date and accept client requestedBy; CSV output lacks formula-injection protection.

Persisted reporting defines scheduled, attempted, connected, completed, opted-out and callback-requested denominators separately.
...
This ticket closes the gap through: replace fixed reports and make exports safe. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Persisted reporting defines scheduled, attempted, connected, completed, opted-out and callback-requested denominators separately. Use authenticated actor, clinic timezone, bounded date ranges and pagination. Export authorization equals on-screen access; audit export metadata without exported payload. Neutralize spreadsheet formulas in text cells and quote CSV correctly.
...
Ticket-specific boundary: Persisted reporting defines scheduled, attempted, connected, completed, opted-out and callback-requested denominators separately. Use authenticated actor, clinic timezone, bounded date ranges and pagination. Export authorization equals on-screen access; audit export metadata without exported payload. Neutralize spreadsheet formulas in text cells and quote CSV correctly.

## 7. Facts, assumptions and unknowns

- **Fact:** Reports use fixed synthetic records/date and accept client requestedBy; CSV output lacks formula-injection protection.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** UI-001
...
Deliver replace fixed reports and make exports safe and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Directory: `src`](../../../src)
- [Directory: `app`](../../../app)
- [Existing file: `docs/analytics.md`](../../../docs/analytics.md)

Proposed output paths (not present merely because this ticket exists):

- `tests/reports.integration.test.cjs`
...
### Step 1: Write metric definitions and fixture truth table

**ACTION:** Write metric definitions and fixture truth table.

**WHY:** The required outcome is: every denominator is reproducible.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Every denominator is reproducible. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Replace fixture queries with canonical bounded queries

**ACTION:** Replace fixture queries with canonical bounded queries.

**WHY:** The required outcome is: report and operational detail reconcile.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Report and operational detail reconcile. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Implement safe CSV serialization and server-derived actor

**ACTION:** Implement safe CSV serialization and server-derived actor.

**WHY:** The required outcome is: formula prefixes and forged requestedBy cannot escape controls.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Formula prefixes and forged requestedBy cannot escape controls. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Add accessible date/filter/download states

**ACTION:** Add accessible date/filter/download states.

**WHY:** The required outcome is: failed exports do not appear successful.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Failed exports do not appear successful. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Every denominator is reproducible.
- [ ] Step 2: Report and operational detail reconcile.
- [ ] Step 3: Formula prefixes and forged requestedBy cannot escape controls.
- [ ] Step 4: Failed exports do not appear successful.

## 16. Error and failure semantics

Inspect the reporting definition and retry inclusion before adjusting totals to match an old demo.
...
### TEST-RPT-001-01: Metric reconciliation

**Purpose:** Prove metric reconciliation under the target contract.
...
**Action:** Exercise `Metric reconciliation` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Mixed outcomes and retries produce exact expected denominators. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Inspect the reporting definition and retry inclusion before adjusting totals to match an old demo.

### TEST-RPT-001-02: CSV hostile values

**Purpose:** Prove cSV hostile values under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `CSV hostile values` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Equals/plus/minus/at-sign, tabs, commas, quotes and newlines cannot become active formulas. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Inspect the reporting definition and retry inclusion before adjusting totals to match an old demo.

### TEST-RPT-001-03: Authorization and outage

**Purpose:** Prove authorization and outage under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Authorization and outage` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Unauthorized download fails before data read; unavailable database returns503. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Mixed outcomes and retries produce exact expected denominators.
- [ ] Equals/plus/minus/at-sign, tabs, commas, quotes and newlines cannot become active formulas.
- [ ] Unauthorized download fails before data read; unavailable database returns503.
...
Disable exports or show an explicit unavailable state; do not restore unlabeled synthetic reports.
...
Notify the implementation owner of AUD-001, REL-001, CBK-005 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
