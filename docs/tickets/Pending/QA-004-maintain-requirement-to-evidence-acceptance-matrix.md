# QA-004: Maintain requirement-to-evidence acceptance matrix

**Status: Pending — Not completed**

**Priority:** P0 · **Area:** I — Governance and evidence · **Type:** Evidence and acceptance
...
Maintain requirement-to-evidence acceptance matrix. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Foundation evidence does not cover the complete three-release scope, and current local/CI/browser/deployment evidence must remain distinct.

Assign stable requirement IDs, release, owner, ticket, risk, test/check, environment, commit, observed result and evidence URL.
...
This ticket closes the gap through: maintain requirement-to-evidence acceptance matrix. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Assign stable requirement IDs, release, owner, ticket, risk, test/check, environment, commit, observed result and evidence URL. Unknown and failed are explicit. This ticket establishes the matrix and review procedure; each release gate refreshes its own rows, avoiding a dependency cycle that would require all releases to finish first.
...
Ticket-specific boundary: Assign stable requirement IDs, release, owner, ticket, risk, test/check, environment, commit, observed result and evidence URL. Unknown and failed are explicit. This ticket establishes the matrix and review procedure; each release gate refreshes its own rows, avoiding a dependency cycle that would require all releases to finish first.

## 7. Facts, assumptions and unknowns

- **Fact:** Foundation evidence does not cover the complete three-release scope, and current local/CI/browser/deployment evidence must remain distinct.
...
Deliver maintain requirement-to-evidence acceptance matrix and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Directory: `docs/tickets`](../../../docs/tickets)
...
- `docs/acceptance/REQUIREMENT_MATRIX.md`
...
### Step 1: Map every approved requirement and ticket to matrix rows

**ACTION:** Map every approved requirement and ticket to matrix rows.

**WHY:** The required outcome is: no in-scope requirement is orphaned.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No in-scope requirement is orphaned. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Separate local, PostgreSQL, browser, hosted CI and staging evidence

**ACTION:** Separate local, PostgreSQL, browser, hosted CI and staging evidence.

**WHY:** The required outcome is: passing one level does not imply the others.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Passing one level does not imply the others. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Define evidence freshness and failure disposition rules

**ACTION:** Define evidence freshness and failure disposition rules.

**WHY:** The required outcome is: changed code invalidates affected old proof.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Changed code invalidates affected old proof. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Review coverage with technical and clinic owners

**ACTION:** Review coverage with technical and clinic owners.

**WHY:** The required outcome is: release sign-off cannot omit unresolved mandatory rows.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Release sign-off cannot omit unresolved mandatory rows. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: No in-scope requirement is orphaned.
- [ ] Step 2: Passing one level does not imply the others.
- [ ] Step 3: Changed code invalidates affected old proof.
- [ ] Step 4: Release sign-off cannot omit unresolved mandatory rows.

## 16. Error and failure semantics

Resolve missing evidence by running the relevant check during implementation, not by copying old status.
...
### TEST-QA-004-01: Coverage audit

**Purpose:** Prove coverage audit under the target contract.
...
**Action:** Exercise `Coverage audit` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Every scoped requirement maps to owner/ticket/check. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Resolve missing evidence by running the relevant check during implementation, not by copying old status.

### TEST-QA-004-02: Evidence boundary

**Purpose:** Prove evidence boundary under the target contract.

**Level:** Artifact/approval review and observed acceptance exercise

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Evidence boundary` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** A local pass cannot mark deployed acceptance passed. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Resolve missing evidence by running the relevant check during implementation, not by copying old status.

### TEST-QA-004-03: Freshness check

**Purpose:** Prove freshness check under the target contract.

**Level:** Artifact/approval review and observed acceptance exercise

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Freshness check` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** A changed contract flags linked stale evidence. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Every scoped requirement maps to owner/ticket/check.
- [ ] A local pass cannot mark deployed acceptance passed.
- [ ] A changed contract flags linked stale evidence.
...
Correct matrix entries with dated history; never delete failed evidence to show a green release.
