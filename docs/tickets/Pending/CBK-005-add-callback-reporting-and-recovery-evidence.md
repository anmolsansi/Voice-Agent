# CBK-005: Add callback reporting and recovery evidence
...
**Depends on:** [CBK-002](CBK-002-deliver-abuse-resistant-public-web-callback-requests.md), [CBK-003](CBK-003-deliver-telephone-callback-request-capture.md), [CBK-004](CBK-004-complete-staff-entered-callback-handling.md), [RPT-001](RPT-001-replace-fixed-reports-and-make-exports-safe.md)

**Blocks:** [REL-003](REL-003-accept-the-complete-standalone-callback-release.md)
...
Add callback reporting and recovery evidence. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Standalone request source and response-time reporting do not exist as verified durable behavior.

Report request count/source, unassigned/overdue work, time to acknowledgment and time to resolution using approved clinic-working-time definitions.
...
This ticket closes the gap through: add callback reporting and recovery evidence. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Report request count/source, unassigned/overdue work, time to acknowledgment and time to resolution using approved clinic-working-time definitions. Never count notification delivery as acknowledgment. Recovery replays preserve one request/work identity. Exports use RPT-001 safety and authorization.
...
Ticket-specific boundary: Report request count/source, unassigned/overdue work, time to acknowledgment and time to resolution using approved clinic-working-time definitions. Never count notification delivery as acknowledgment. Recovery replays preserve one request/work identity. Exports use RPT-001 safety and authorization.

## 7. Facts, assumptions and unknowns

- **Fact:** Standalone request source and response-time reporting do not exist as verified durable behavior.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** CBK-002, CBK-003, CBK-004, RPT-001
...
Deliver add callback reporting and recovery evidence and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `tests/callback-reporting.integration.test.cjs`
...
### Step 1: Define metric denominators and working-time clocks

**ACTION:** Define metric denominators and working-time clocks.

**WHY:** The required outcome is: staff can reproduce service metrics.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Staff can reproduce service metrics. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Query all sources using canonical requests/work

**ACTION:** Query all sources using canonical requests/work.

**WHY:** The required outcome is: web/phone/staff counts reconcile.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Web/phone/staff counts reconcile. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Exercise restart/restore/replay scenarios

**ACTION:** Exercise restart/restore/replay scenarios.

**WHY:** The required outcome is: no request disappears or doubles.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No request disappears or doubles. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Publish failure/recovery and reporting evidence

**ACTION:** Publish failure/recovery and reporting evidence.

**WHY:** The required outcome is: operational blind spots are explicit.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Operational blind spots are explicit. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Staff can reproduce service metrics.
- [ ] Step 2: Web/phone/staff counts reconcile.
- [ ] Step 3: No request disappears or doubles.
- [ ] Step 4: Operational blind spots are explicit.

## 16. Error and failure semantics

Inspect request timestamps and clinic-calendar version when response times disagree.
...
### TEST-CBK-005-01: Mixed source truth table

**Purpose:** Prove mixed source truth table under the target contract.
...
**Action:** Exercise `Mixed source truth table` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Exact counts and response intervals match fixtures. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Inspect request timestamps and clinic-calendar version when response times disagree.

### TEST-CBK-005-02: Restart/replay

**Purpose:** Prove restart/replay under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Restart/replay` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Stable request/work identities and accurate counts. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Inspect request timestamps and clinic-calendar version when response times disagree.

### TEST-CBK-005-03: Export

**Purpose:** Prove export under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Export` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Role and formula protections remain active. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Exact counts and response intervals match fixtures.
- [ ] Stable request/work identities and accurate counts.
- [ ] Role and formula protections remain active.
...
Disable incorrect reports and retain queue operations; never alter source records to improve metrics.
...
Notify the implementation owner of REL-003 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
