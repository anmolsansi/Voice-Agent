# VOI-001: Build authenticated bounded telephone audio transport
...
**Depends on:** [TEL-003](TEL-003-dispatch-cancel-and-reconcile-uncertain-provider-outcomes.md)

**Blocks:** [VOI-002](VOI-002-implement-approved-fixed-script-conversation-behavior.md)
...
Build authenticated bounded telephone audio transport. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Prototype turn handlers exist but there is no authenticated end-to-end telephone media transport in the operational backend.

Secure WebSocket media ingress binds verified provider/account/call/stream IDs to one attempt.
...
This ticket closes the gap through: build authenticated bounded telephone audio transport. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Secure WebSocket media ingress binds verified provider/account/call/stream IDs to one attempt. Apply selected codec contract, frame limits, sequence checks, bounded queues, connection/time limits and backpressure. Initial queue cap2seconds audio; overflow ends/degrades safely with explicit outcome. Count reservations across workflows; do not write audio files or enable provider recordings.
...
Ticket-specific boundary: Secure WebSocket media ingress binds verified provider/account/call/stream IDs to one attempt. Apply selected codec contract, frame limits, sequence checks, bounded queues, connection/time limits and backpressure. Initial queue cap2seconds audio; overflow ends/degrades safely with explicit outcome. Count reservations across workflows; do not write audio files or enable provider recordings.

## 7. Facts, assumptions and unknowns

- **Fact:** Prototype turn handlers exist but there is no authenticated end-to-end telephone media transport in the operational backend.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** TEL-003
...
Deliver build authenticated bounded telephone audio transport and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Existing file: `src/voice/realtime/turn-handler.mjs`](../../../src/voice/realtime/turn-handler.mjs)
...
- `src/modules/telephony/media.js`
- `tests/media-transport.test.cjs`
...
### Step 1: Implement provider connection verification and attempt binding

**ACTION:** Implement provider connection verification and attempt binding.

**WHY:** The required outcome is: a guessed call ID cannot attach an audio stream.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** A guessed call ID cannot attach an audio stream. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Validate codec/frame envelope and sequence before decoding

**ACTION:** Validate codec/frame envelope and sequence before decoding.

**WHY:** The required outcome is: malformed frames do not allocate unbounded buffers.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Malformed frames do not allocate unbounded buffers. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Bridge audio to the selected speech pipeline with cancellation/backpressure

**ACTION:** Bridge audio to the selected speech pipeline with cancellation/backpressure.

**WHY:** The required outcome is: interruption clears obsolete output and bounded memory is measurable.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Interruption clears obsolete output and bounded memory is measurable. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Handle silence, disconnect and graceful shutdown

**ACTION:** Handle silence, disconnect and graceful shutdown.

**WHY:** The required outcome is: media closure generates a recoverable outcome, not false business completion.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Media closure generates a recoverable outcome, not false business completion. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: A guessed call ID cannot attach an audio stream.
- [ ] Step 2: Malformed frames do not allocate unbounded buffers.
- [ ] Step 3: Interruption clears obsolete output and bounded memory is measurable.
- [ ] Step 4: Media closure generates a recoverable outcome, not false business completion.

## 16. Error and failure semantics

Measure queue depth and reservation leaks; a WebSocket disconnect alone cannot prove the telephone attempt is terminal.
...
### TEST-VOI-001-01: Unauthorized/cross-call stream

**Purpose:** Prove unauthorized/cross-call stream under the target contract.
...
**Action:** Exercise `Unauthorized/cross-call stream` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Connection rejected and no transcript/answer mutation. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Measure queue depth and reservation leaks; a WebSocket disconnect alone cannot prove the telephone attempt is terminal.

### TEST-VOI-001-02: Frame flood

**Purpose:** Prove frame flood under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Frame flood` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Buffer cap holds and stream fails safely without process memory growth. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Measure queue depth and reservation leaks; a WebSocket disconnect alone cannot prove the telephone attempt is terminal.

### TEST-VOI-001-03: Interruption/disconnect

**Purpose:** Prove interruption/disconnect under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Interruption/disconnect` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Obsolete speech stops; active reservation releases after terminal/reconciliation policy. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Measure queue depth and reservation leaks; a WebSocket disconnect alone cannot prove the telephone attempt is terminal.

### TEST-VOI-001-04: Recording disabled

**Purpose:** Prove recording disabled under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Recording disabled` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** No audio artifact/provider recording request is created. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Connection rejected and no transcript/answer mutation.
- [ ] Buffer cap holds and stream fails safely without process memory growth.
- [ ] Obsolete speech stops; active reservation releases after terminal/reconciliation policy.
- [ ] No audio artifact/provider recording request is created.
...
Disable media acceptance and stop/drain controlled calls; preserve attempts and reconciliation state.
...
Notify the implementation owner of VOI-002 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
