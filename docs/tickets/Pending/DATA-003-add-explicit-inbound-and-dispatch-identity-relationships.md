# DATA-003: Add explicit inbound and dispatch identity relationships
...
**Depends on:** [TEL-001](TEL-001-define-provider-adapters-and-canonical-event-semantics.md)

**Blocks:** [TEL-002](TEL-002-verify-and-durably-receive-provider-events.md)
...
Add explicit inbound and dispatch identity relationships. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

All current attempts require a patient and schedule. A real inbound intake/callback may begin with an unverified caller and no outbound schedule.

Add direction and purpose with database CHECK constraints.
...
This ticket closes the gap through: add explicit inbound and dispatch identity relationships. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Add direction and purpose with database CHECK constraints. Existing attempts remain outbound/checkin with valid patient/schedule/occurrence relationships. Inbound attempts may initially have null patient/schedule and must reference a unique provider/account/call identity; linking a patient later requires verified staff/patient authority. Add durable dispatch_operations and provider-event receipt uniqueness. Public POST /api/calls keeps its outbound contract; provider ingress uses an internal inbound creator.
...
Ticket-specific boundary: Add direction and purpose with database CHECK constraints. Existing attempts remain outbound/checkin with valid patient/schedule/occurrence relationships. Inbound attempts may initially have null patient/schedule and must reference a unique provider/account/call identity; linking a patient later requires verified staff/patient authority. Add durable dispatch_operations and provider-event receipt uniqueness. Public POST /api/calls keeps its outbound contract; provider ingress uses an internal inbound creator.

## 7. Facts, assumptions and unknowns

- **Fact:** All current attempts require a patient and schedule. A real inbound intake/callback may begin with an unverified caller and no outbound schedule.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** TEL-001
...
Deliver add explicit inbound and dispatch identity relationships and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- [Existing file: `db/migrations/006_canonical_calls.cjs`](../../../db/migrations/006_canonical_calls.cjs)
- [Existing file: `src/modules/calls/service.js`](../../../src/modules/calls/service.js)
- [Existing file: `tests/migrations.integration.test.cjs`](../../../tests/migrations.integration.test.cjs)

Proposed output paths (not present merely because this ticket exists):

- `tests/inbound-call-model.integration.test.cjs`
...
### Step 1: Classify existing attempts and constraints

**ACTION:** Classify existing attempts and constraints.

**WHY:** The required outcome is: no legacy row is reclassified as inbound to bypass validation.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No legacy row is reclassified as inbound to bypass validation. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Add direction-aware constraints and unique provider identity

**ACTION:** Add direction-aware constraints and unique provider identity.

**WHY:** The required outcome is: outbound invariants survive while inbound unknown identity is representable.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Outbound invariants survive while inbound unknown identity is representable. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Implement separate internal inbound creator and audited verified linking

**ACTION:** Implement separate internal inbound creator and audited verified linking.

**WHY:** The required outcome is: caller ID never grants patient identity.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Caller ID never grants patient identity. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Add migration upgrade/rollback compatibility tests

**ACTION:** Add migration upgrade/rollback compatibility tests.

**WHY:** The required outcome is: new inbound rows are not fed into older code that assumes a schedule.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** New inbound rows are not fed into older code that assumes a schedule. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: No legacy row is reclassified as inbound to bypass validation.
- [ ] Step 2: Outbound invariants survive while inbound unknown identity is representable.
- [ ] Step 3: Caller ID never grants patient identity.
- [ ] Step 4: New inbound rows are not fed into older code that assumes a schedule.

## 16. Error and failure semantics

Do not create dummy patients or schedules to satisfy old not-null constraints; fix the explicit direction model.
...
### TEST-DATA-003-01: Inbound unknown caller

**Purpose:** Prove inbound unknown caller under the target contract.
...
**Action:** Exercise `Inbound unknown caller` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Valid unlinked attempt without fake patient or schedule. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Do not create dummy patients or schedules to satisfy old not-null constraints; fix the explicit direction model.

### TEST-DATA-003-02: Outbound missing identity

**Purpose:** Prove outbound missing identity under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Outbound missing identity` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Constraint/API rejection still occurs. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Do not create dummy patients or schedules to satisfy old not-null constraints; fix the explicit direction model.

### TEST-DATA-003-03: Duplicate provider callback

**Purpose:** Prove duplicate provider callback under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Duplicate provider callback` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Same inbound attempt, not a duplicate patient. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Do not create dummy patients or schedules to satisfy old not-null constraints; fix the explicit direction model.

### TEST-DATA-003-04: Unauthorized linking

**Purpose:** Prove unauthorized linking under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Unauthorized linking` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** No patient relationship changes when caller supplies a guessed patient ID. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Valid unlinked attempt without fake patient or schedule.
- [ ] Constraint/API rejection still occurs.
- [ ] Same inbound attempt, not a duplicate patient.
- [ ] No patient relationship changes when caller supplies a guessed patient ID.
...
Disable inbound entrypoints and retain new rows; roll back only to direction-aware compatible code.
...
Notify the implementation owner of TEL-002 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
