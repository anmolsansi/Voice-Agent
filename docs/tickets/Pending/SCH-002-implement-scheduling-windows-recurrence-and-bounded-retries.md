# SCH-002: Implement scheduling windows recurrence and bounded retries
...
**Depends on:** [SCH-001](SCH-001-model-post-visit-occurrences-and-immutable-script-versions.md), [DEC-004](DEC-004-approve-clinic-hours-follow-up-and-safety-responsibilities.md)

**Blocks:** [SCH-003](SCH-003-claim-due-work-fairly-with-leases-and-concurrency-limits.md)
...
Implement scheduling windows recurrence and bounded retries. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

The enqueue job uses persisted next_due_at and a repeat-safe key, but no complete staff scheduling/window/advancement workflow exists.

Staff /api/checkin-schedules create/read/update supports patient, approved script, IANA timezone, due time, window and state.
...
This ticket closes the gap through: implement scheduling windows recurrence and bounded retries. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Staff /api/checkin-schedules create/read/update supports patient, approved script, IANA timezone, due time, window and state. Retry policy is a versioned approved clinic setting, not arbitrary request metadata. All instants persist UTC; evaluate local windows with IANA timezone. Dispatch outside a window is deferred, not discarded. Completing/canceling/exhausting an occurrence advances or closes the schedule atomically.
...
Ticket-specific boundary: Staff /api/checkin-schedules create/read/update supports patient, approved script, IANA timezone, due time, window and state. Retry policy is a versioned approved clinic setting, not arbitrary request metadata. All instants persist UTC; evaluate local windows with IANA timezone. Dispatch outside a window is deferred, not discarded. Completing/canceling/exhausting an occurrence advances or closes the schedule atomically.

## 7. Facts, assumptions and unknowns

- **Fact:** The enqueue job uses persisted next_due_at and a repeat-safe key, but no complete staff scheduling/window/advancement workflow exists.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** SCH-001, DEC-004
...
Deliver implement scheduling windows recurrence and bounded retries and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `src/modules/schedules/routes.js`
- `src/modules/schedules/policy.js`
- `tests/scheduling.integration.test.cjs`
...
### Step 1: Implement validated schedule DTO and staff controls

**ACTION:** Implement validated schedule DTO and staff controls.

**WHY:** The required outcome is: invalid timezone/window and unapproved script fail before write.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Invalid timezone/window and unapproved script fail before write. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Define approved retryable outcomes and maximum attempts per occurrence

**ACTION:** Define approved retryable outcomes and maximum attempts per occurrence.

**WHY:** The required outcome is: unlimited retries are impossible.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Unlimited retries are impossible. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Compute DST/holiday/missed-window behavior from policy

**ACTION:** Compute DST/holiday/missed-window behavior from policy.

**WHY:** The required outcome is: each edge case has a deterministic next eligible instant.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Each edge case has a deterministic next eligible instant. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Advance occurrence/schedule with mandatory audit in one transaction

**ACTION:** Advance occurrence/schedule with mandatory audit in one transaction.

**WHY:** The required outcome is: already-handled due rows do not block later work.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Already-handled due rows do not block later work. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 5: Expose blocked/deferred/exhausted reasons to staff

**ACTION:** Expose blocked/deferred/exhausted reasons to staff.

**WHY:** The required outcome is: a silent skip is not presented as successful outreach.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** A silent skip is not presented as successful outreach. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Invalid timezone/window and unapproved script fail before write.
- [ ] Step 2: Unlimited retries are impossible.
- [ ] Step 3: Each edge case has a deterministic next eligible instant.
- [ ] Step 4: Already-handled due rows do not block later work.
- [ ] Step 5: A silent skip is not presented as successful outreach.

## 16. Error and failure semantics

Check occurrence identity and next_due advancement before increasing list limits; a limit increase alone does not fix starvation.
...
### TEST-SCH-002-01: DST and holiday fixtures

**Purpose:** Prove dST and holiday fixtures under the target contract.
...
**Action:** Exercise `DST and holiday fixtures` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Correct UTC dispatch instant and no double occurrence during repeated local time. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Check occurrence identity and next_due advancement before increasing list limits; a limit increase alone does not fix starvation.

### TEST-SCH-002-02: Retry exhaustion

**Purpose:** Prove retry exhaustion under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Retry exhaustion` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** No extra attempt after the approved cap; staff work is created if policy requires. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Check occurrence identity and next_due advancement before increasing list limits; a limit increase alone does not fix starvation.

### TEST-SCH-002-03: Pause/reschedule race

**Purpose:** Prove pause/reschedule race under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Pause/reschedule race` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Old pending occurrence cannot dispatch after authoritative pause. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Check occurrence identity and next_due advancement before increasing list limits; a limit increase alone does not fix starvation.

### TEST-SCH-002-04: Repeated enqueue

**Purpose:** Prove repeated enqueue under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Repeated enqueue` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Stable IDs and counts without resetting retry history. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Correct UTC dispatch instant and no double occurrence during repeated local time.
- [ ] No extra attempt after the approved cap; staff work is created if policy requires.
- [ ] Old pending occurrence cannot dispatch after authoritative pause.
- [ ] Stable IDs and counts without resetting retry history.
...
Pause schedules; retain due/history and restart from recorded occurrence state after correction.
...
Notify the implementation owner of SCH-003 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
