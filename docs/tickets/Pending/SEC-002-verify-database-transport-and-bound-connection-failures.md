# SEC-002: Verify database transport and bound connection failures
...
**Depends on:** [SEC-001](SEC-001-resolve-dependency-vulnerabilities-with-regression-proof.md)

**Blocks:** [SEC-003](SEC-003-verify-proxy-trust-login-limits-and-hash-capacity.md), [OPS-001](OPS-001-package-one-clinic-on-a-low-fixed-cost-vm.md)
...
Verify database transport and bound connection failures. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

PGSSL currently enables ssl with rejectUnauthorized:false. Pool connection and statement limits are not explicit in the inspected factory.

Network PostgreSQL TLS must verify the server certificate using a configured CA or system trust.
...
This ticket closes the gap through: verify database transport and bound connection failures. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Network PostgreSQL TLS must verify the server certificate using a configured CA or system trust. Local isolated test transport may be non-TLS by explicit environment policy. Add bounded connect/query timeouts and a documented per-process pool budget. Production must reject an insecure TLS-bypass option.
...
Ticket-specific boundary: Network PostgreSQL TLS must verify the server certificate using a configured CA or system trust. Local isolated test transport may be non-TLS by explicit environment policy. Add bounded connect/query timeouts and a documented per-process pool budget. Production must reject an insecure TLS-bypass option.

## 7. Facts, assumptions and unknowns

- **Fact:** PGSSL currently enables ssl with rejectUnauthorized:false. Pool connection and statement limits are not explicit in the inspected factory.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** SEC-001
...
Deliver verify database transport and bound connection failures and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `tests/database-transport.integration.test.cjs`
...
### Step 1: Inventory every pool consumer and deployment connection path

**ACTION:** Inventory every pool consumer and deployment connection path.

**WHY:** The required outcome is: backend, workers and migration tools have separate documented budgets.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Backend, workers and migration tools have separate documented budgets. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Add verified TLS/CA configuration and reject invalid startup values

**ACTION:** Add verified TLS/CA configuration and reject invalid startup values.

**WHY:** The required outcome is: a certificate mismatch fails before a domain operation.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** A certificate mismatch fails before a domain operation. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Set initial connect timeout5s and statement timeout10s, with bounded pools

**ACTION:** Set initial connect timeout5s and statement timeout10s, with bounded pools.

**WHY:** The required outcome is: outages terminate predictably; long maintenance operations use a separate operator connection.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Outages terminate predictably; long maintenance operations use a separate operator connection. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Verify failures through readiness and protected APIs

**ACTION:** Verify failures through readiness and protected APIs.

**WHY:** The required outcome is: database failure returns503 without cached durable success.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Database failure returns503 without cached durable success. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Backend, workers and migration tools have separate documented budgets.
- [ ] Step 2: A certificate mismatch fails before a domain operation.
- [ ] Step 3: Outages terminate predictably; long maintenance operations use a separate operator connection.
- [ ] Step 4: Database failure returns503 without cached durable success.

## 16. Error and failure semantics

Separate certificate/hostname failure from exhausted connections. Never solve a trust failure by disabling verification.
...
### TEST-SEC-002-01: Untrusted database certificate

**Purpose:** Prove untrusted database certificate under the target contract.
...
**Action:** Exercise `Untrusted database certificate` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Connection fails; no query mutation and no automatic insecure retry occur. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Separate certificate/hostname failure from exhausted connections. Never solve a trust failure by disabling verification.

### TEST-SEC-002-02: Trusted CA

**Purpose:** Prove trusted CA under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Trusted CA` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Configured network connection succeeds with certificate verification enabled. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Separate certificate/hostname failure from exhausted connections. Never solve a trust failure by disabling verification.

### TEST-SEC-002-03: Pool saturation/outage

**Purpose:** Prove pool saturation/outage under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Pool saturation/outage` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Requests fail within the configured bound; connections are released and readiness is unhealthy. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Connection fails; no query mutation and no automatic insecure retry occur.
- [ ] Configured network connection succeeds with certificate verification enabled.
- [ ] Requests fail within the configured bound; connections are released and readiness is unhealthy.
...
Restore the previous verified CA/configuration or disable service while correcting trust; do not restore rejectUnauthorized:false.
...
Notify the implementation owner of SEC-003, OPS-001 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
