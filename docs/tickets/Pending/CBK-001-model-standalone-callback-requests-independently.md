# CBK-001: Model standalone callback requests independently

**Status: Pending — Not completed**

**Priority:** P0 · **Area:** H — Release 3 callbacks · **Type:** Engineering

**Complexity:** Medium (planning band; estimate after reconnaissance)

**Depends on:** [WRK-001](WRK-001-persist-one-shared-follow-up-work-item-lifecycle.md), [REL-002](REL-002-accept-complete-browser-and-telephone-intake-release.md)

**Blocks:** [CBK-002](CBK-002-deliver-abuse-resistant-public-web-callback-requests.md), [CBK-003](CBK-003-deliver-telephone-callback-request-capture.md), [CBK-004](CBK-004-complete-staff-entered-callback-handling.md)
...
Model standalone callback requests independently. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Release1 work items support check-in follow-up but standalone requests need independent source, identity and lifecycle contracts.

A callback request is independent of a check-in attempt and may begin with an unverified contact.
...
This ticket closes the gap through: model standalone callback requests independently. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
A callback request is independent of a check-in attempt and may begin with an unverified contact. Associate patient only after verification; retain source provenance and requested callback window. Use the existing staff work lifecycle for assignment/acknowledgment/resolution. Duplicate detection suggests review, never merges people by phone/name.
...
Ticket-specific boundary: A callback request is independent of a check-in attempt and may begin with an unverified contact. Associate patient only after verification; retain source provenance and requested callback window. Use the existing staff work lifecycle for assignment/acknowledgment/resolution. Duplicate detection suggests review, never merges people by phone/name.

## 7. Facts, assumptions and unknowns

- **Fact:** Release1 work items support check-in follow-up but standalone requests need independent source, identity and lifecycle contracts.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** WRK-001, REL-002
...
Deliver model standalone callback requests independently and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `tests/callbacks.integration.test.cjs`
...
### Step 1: Define standalone request schema and source contracts

**ACTION:** Define standalone request schema and source contracts.

**WHY:** The required outcome is: no fake call attempt or patient is required.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No fake call attempt or patient is required. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Add transactional request/work creation

**ACTION:** Add transactional request/work creation.

**WHY:** The required outcome is: one logical request creates one durable work item.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** One logical request creates one durable work item. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Define verification and safe association action

**ACTION:** Define verification and safe association action.

**WHY:** The required outcome is: contact information is not treated as identity.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Contact information is not treated as identity. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Document acknowledgment and resolution semantics

**ACTION:** Document acknowledgment and resolution semantics.

**WHY:** The required outcome is: delivery receipt is not staff acknowledgment.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Delivery receipt is not staff acknowledgment. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: No fake call attempt or patient is required.
- [ ] Step 2: One logical request creates one durable work item.
- [ ] Step 3: Contact information is not treated as identity.
- [ ] Step 4: Delivery receipt is not staff acknowledgment.

## 16. Error and failure semantics

Separate inbound contact facts from verified patient relationships.
...
### TEST-CBK-001-01: Standalone request

**Purpose:** Prove standalone request under the target contract.
...
**Action:** Exercise `Standalone request` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Persists with no check-in attempt. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Separate inbound contact facts from verified patient relationships.

### TEST-CBK-001-02: Replay

**Purpose:** Prove replay under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Replay` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Same idempotency input returns same request/work. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Separate inbound contact facts from verified patient relationships.

### TEST-CBK-001-03: Identity conflict

**Purpose:** Prove identity conflict under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Identity conflict` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Existing patient is not silently linked by phone. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Persists with no check-in attempt.
- [ ] Same idempotency input returns same request/work.
- [ ] Existing patient is not silently linked by phone.
...
Disable new standalone request creation while preserving existing follow-up queues.
...
Notify the implementation owner of CBK-002, CBK-003, CBK-004 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
