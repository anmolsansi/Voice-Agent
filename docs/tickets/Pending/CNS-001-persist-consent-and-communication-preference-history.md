# CNS-001: Persist consent and communication preference history
...
**Depends on:** [PAT-002](PAT-002-persist-guardian-relationships-and-enforce-authority.md), [DEC-002](DEC-002-approve-launch-country-languages-and-data-boundaries.md)

**Blocks:** [CNS-002](CNS-002-enforce-opt-out-immediately-before-every-dispatch.md), [SCH-001](SCH-001-model-post-visit-occurrences-and-immutable-script-versions.md)
...
Persist consent and communication preference history. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Prototype consentStatus and intake treatment acknowledgments exist, but durable outreach permission/history is absent.

Add append-only consent_events with subject patient, authorized respondent, channel, scope, policy version, granted/revoked action, effective timestamp, source and server actor.
...
This ticket closes the gap through: persist consent and communication preference history. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Add append-only consent_events with subject patient, authorized respondent, channel, scope, policy version, granted/revoked action, effective timestamp, source and server actor. Store wrong-number, pause and contact restrictions explicitly. GET/POST /api/patients/:id/consents is staff authorized; voice opt-out uses an internal service context. Treatment consent is not phone outreach consent.
...
Ticket-specific boundary: Add append-only consent_events with subject patient, authorized respondent, channel, scope, policy version, granted/revoked action, effective timestamp, source and server actor. Store wrong-number, pause and contact restrictions explicitly. GET/POST /api/patients/:id/consents is staff authorized; voice opt-out uses an internal service context. Treatment consent is not phone outreach consent.

## 7. Facts, assumptions and unknowns

- **Fact:** Prototype consentStatus and intake treatment acknowledgments exist, but durable outreach permission/history is absent.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** PAT-002, DEC-002
...
Deliver persist consent and communication preference history and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `src/modules/consent/service.js`
- `src/modules/consent/routes.js`
- `tests/consent.integration.test.cjs`
...
### Step 1: Map approved policy scopes and contact targets

**ACTION:** Map approved policy scopes and contact targets.

**WHY:** The required outcome is: each permission answers what communication is allowed for whom.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Each permission answers what communication is allowed for whom. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Add event history and authoritative current eligibility query

**ACTION:** Add event history and authoritative current eligibility query.

**WHY:** The required outcome is: revocation is preserved rather than overwritten.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Revocation is preserved rather than overwritten. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Implement staff change flow with role/guardian checks and audit

**ACTION:** Implement staff change flow with role/guardian checks and audit.

**WHY:** The required outcome is: caller cannot grant authority by choosing an actor.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Caller cannot grant authority by choosing an actor. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Expose a safe eligibility reason in patient/schedule views

**ACTION:** Expose a safe eligibility reason in patient/schedule views.

**WHY:** The required outcome is: missing consent is visible as blocked, not silently skipped.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Missing consent is visible as blocked, not silently skipped. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Each permission answers what communication is allowed for whom.
- [ ] Step 2: Revocation is preserved rather than overwritten.
- [ ] Step 3: Caller cannot grant authority by choosing an actor.
- [ ] Step 4: Missing consent is visible as blocked, not silently skipped.

## 16. Error and failure semantics

Inspect scope/channel/subject separately; a single global consent boolean cannot represent the approved policy.
...
### TEST-CNS-001-01: Treatment-only consent

**Purpose:** Prove treatment-only consent under the target contract.
...
**Action:** Exercise `Treatment-only consent` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Does not authorize phone outreach. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Inspect scope/channel/subject separately; a single global consent boolean cannot represent the approved policy.

### TEST-CNS-001-02: Revocation ordering

**Purpose:** Prove revocation ordering under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Revocation ordering` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Latest effective applicable event suppresses outreach after restart. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Inspect scope/channel/subject separately; a single global consent boolean cannot represent the approved policy.

### TEST-CNS-001-03: Unauthorized guardian grant

**Purpose:** Prove unauthorized guardian grant under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Unauthorized guardian grant` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Rejected with consent history unchanged. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Inspect scope/channel/subject separately; a single global consent boolean cannot represent the approved policy.

### TEST-CNS-001-04: Audit rollback

**Purpose:** Prove audit rollback under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Audit rollback` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Failed required audit leaves no new consent event. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Does not authorize phone outreach.
- [ ] Latest effective applicable event suppresses outreach after restart.
- [ ] Rejected with consent history unchanged.
- [ ] Failed required audit leaves no new consent event.
...
Disable grants or outreach while preserving revocations; never restore eligibility from stale cached booleans.
...
Notify the implementation owner of CNS-002, SCH-001 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
