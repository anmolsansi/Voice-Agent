# INT-002: Complete patient and guardian access, expiry and resume
...
**Depends on:** [INT-001](INT-001-define-one-versioned-intake-contract-for-both-voice-channels.md), [DEC-003](DEC-003-approve-adult-minor-and-guardian-authority-rules.md)

**Blocks:** [INT-003](INT-003-deliver-browser-microphone-intake-with-review.md), [INT-004](INT-004-deliver-verified-telephone-intake.md)
...
Complete patient and guardian access, expiry and resume. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Hardened capabilities alone do not prove clinically approved patient/guardian access, invite expiry and resumption.

Implement approved assurance and guardian policies from DEC-003.
...
This ticket closes the gap through: complete patient and guardian access, expiry and resume. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Implement approved assurance and guardian policies from DEC-003. Links are scoped, expiring, revocable and do not expose patient data before required verification. Resume returns the correct draft/schema and revision; expired or replaced invitations cannot access it. No phone number is treated as proof of patient identity. Public failure text avoids account enumeration.
...
Ticket-specific boundary: Implement approved assurance and guardian policies from DEC-003. Links are scoped, expiring, revocable and do not expose patient data before required verification. Resume returns the correct draft/schema and revision; expired or replaced invitations cannot access it. No phone number is treated as proof of patient identity. Public failure text avoids account enumeration.

## 7. Facts, assumptions and unknowns

- **Fact:** Hardened capabilities alone do not prove clinically approved patient/guardian access, invite expiry and resumption.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** INT-001, DEC-003
...
Deliver complete patient and guardian access, expiry and resume and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
### Step 1: Translate approved verification policy into explicit states

**ACTION:** Translate approved verification policy into explicit states.

**WHY:** The required outcome is: disclosure is impossible before required assurance.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Disclosure is impossible before required assurance. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Implement invite/revoke/resend and resume contracts

**ACTION:** Implement invite/revoke/resend and resume contracts.

**WHY:** The required outcome is: old token revocation is immediate.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Old token revocation is immediate. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Bind verified respondent authority to draft

**ACTION:** Bind verified respondent authority to draft.

**WHY:** The required outcome is: guardian scope is auditable.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Guardian scope is auditable. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Add expiry/recovery UX without silent data loss

**ACTION:** Add expiry/recovery UX without silent data loss.

**WHY:** The required outcome is: staff can safely reissue access.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Staff can safely reissue access. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Disclosure is impossible before required assurance.
- [ ] Step 2: Old token revocation is immediate.
- [ ] Step 3: Guardian scope is auditable.
- [ ] Step 4: Staff can safely reissue access.

## 16. Error and failure semantics

Separate capability possession, identity verification and guardian authority when diagnosing denial.
...
### TEST-INT-002-01: Replaced invite

**Purpose:** Prove replaced invite under the target contract.
...
**Action:** Exercise `Replaced invite` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Old capability cannot read or write. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Separate capability possession, identity verification and guardian authority when diagnosing denial.

### TEST-INT-002-02: Wrong guardian

**Purpose:** Prove wrong guardian under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Wrong guardian` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** No patient fields disclosed. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Separate capability possession, identity verification and guardian authority when diagnosing denial.

### TEST-INT-002-03: Resume after restart

**Purpose:** Prove resume after restart under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Resume after restart` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Last committed revision and schema return correctly. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Old capability cannot read or write.
- [ ] No patient fields disclosed.
- [ ] Last committed revision and schema return correctly.
...
Disable invitations and use approved staff-assisted intake until access assurance is repaired.
...
Notify the implementation owner of INT-003, INT-004 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
