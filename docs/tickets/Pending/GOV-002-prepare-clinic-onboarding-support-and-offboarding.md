# GOV-002: Prepare clinic onboarding, support and offboarding
...
**Depends on:** [OPS-004](OPS-004-establish-an-isolated-synthetic-staging-environment.md), [GOV-001](GOV-001-define-and-implement-retention-access-and-deletion-policy.md)

**Blocks:** [REL-001](REL-001-accept-the-complete-post-visit-check-in-release.md), [REL-002](REL-002-accept-complete-browser-and-telephone-intake-release.md), [REL-003](REL-003-accept-the-complete-standalone-callback-release.md)
...
Prepare clinic onboarding, support and offboarding. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

There is no buyer-approved onboarding/support/offboarding package or deployed handover.

Document clinic isolation, contacts, administrator bootstrap/MFA/recovery, patient import/consent verification, approved scripts, hours, staff follow-up, incident ownership and cost review.
...
This ticket closes the gap through: prepare clinic onboarding, support and offboarding. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Document clinic isolation, contacts, administrator bootstrap/MFA/recovery, patient import/consent verification, approved scripts, hours, staff follow-up, incident ownership and cost review. Offboarding revokes access/credentials, stops schedules, handles open work and approved export/retention/deletion. Support is clinic-hours as agreed; infrastructure incidents have explicitly named operator responsibility.
...
Ticket-specific boundary: Document clinic isolation, contacts, administrator bootstrap/MFA/recovery, patient import/consent verification, approved scripts, hours, staff follow-up, incident ownership and cost review. Offboarding revokes access/credentials, stops schedules, handles open work and approved export/retention/deletion. Support is clinic-hours as agreed; infrastructure incidents have explicitly named operator responsibility.

## 7. Facts, assumptions and unknowns

- **Fact:** There is no buyer-approved onboarding/support/offboarding package or deployed handover.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** OPS-004, GOV-001
...
Deliver prepare clinic onboarding, support and offboarding and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `docs/operations/clinic-lifecycle.md`
...
### Step 1: Create onboarding checklist and training scenarios

**ACTION:** Create onboarding checklist and training scenarios.

**WHY:** The required outcome is: named staff can operate complete scoped workflow.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Named staff can operate complete scoped workflow. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Rehearse access recovery and incident escalation

**ACTION:** Rehearse access recovery and incident escalation.

**WHY:** The required outcome is: responsibility is explicit outside clinic hours.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Responsibility is explicit outside clinic hours. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Document offboarding order and evidence

**ACTION:** Document offboarding order and evidence.

**WHY:** The required outcome is: outbound activity stops before account/data teardown.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Outbound activity stops before account/data teardown. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Obtain clinic/operator acceptance of handover

**ACTION:** Obtain clinic/operator acceptance of handover.

**WHY:** The required outcome is: no unowned alerts or recovery duties remain.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No unowned alerts or recovery duties remain. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Named staff can operate complete scoped workflow.
- [ ] Step 2: Responsibility is explicit outside clinic hours.
- [ ] Step 3: Outbound activity stops before account/data teardown.
- [ ] Step 4: No unowned alerts or recovery duties remain.

## 16. Error and failure semantics

An unsigned checklist does not prove staff coverage or operator availability.
...
### TEST-GOV-002-01: New clinic rehearsal

**Purpose:** Prove new clinic rehearsal under the target contract.
...
**Action:** Exercise `New clinic rehearsal` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Isolated synthetic instance reaches trained operational readiness. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** An unsigned checklist does not prove staff coverage or operator availability.

### TEST-GOV-002-02: Offboarding rehearsal

**Purpose:** Prove offboarding rehearsal under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Offboarding rehearsal` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** No further dispatch; open work is handed over; sessions revoked. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** An unsigned checklist does not prove staff coverage or operator availability.

### TEST-GOV-002-03: Incident exercise

**Purpose:** Prove incident exercise under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Incident exercise` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Correct owner receives safe context and follows runbook. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Isolated synthetic instance reaches trained operational readiness.
- [ ] No further dispatch; open work is handed over; sessions revoked.
- [ ] Correct owner receives safe context and follows runbook.
...
Delay onboarding or pause outreach until ownership and safe handover are restored.
...
Notify the implementation owner of REL-001, REL-002, REL-003 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
