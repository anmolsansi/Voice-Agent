# UI-002: Complete patient and call review screens
...
Complete patient and call review screens. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Existing screens do not establish a complete persisted patient-to-attempt-to-follow-up review journey.

Patient detail shows verified contacts, guardian scope, consent and schedules.
...
This ticket closes the gap through: complete patient and call review screens. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Patient detail shows verified contacts, guardian scope, consent and schedules. Attempt detail shows canonical lifecycle, script version, answer provenance and linked work. Missing/late transcript is explicit. Ordinary staff cannot invoke admin-only call actions. Sensitive content must not enter URL query parameters or browser analytics.
...
Ticket-specific boundary: Patient detail shows verified contacts, guardian scope, consent and schedules. Attempt detail shows canonical lifecycle, script version, answer provenance and linked work. Missing/late transcript is explicit. Ordinary staff cannot invoke admin-only call actions. Sensitive content must not enter URL query parameters or browser analytics.

## 7. Facts, assumptions and unknowns

- **Fact:** Existing screens do not establish a complete persisted patient-to-attempt-to-follow-up review journey.
...
Deliver complete patient and call review screens and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `tests/browser/call-review.spec.ts`
...
### Step 1: Build patient history with bounded pagination

**ACTION:** Build patient history with bounded pagination.

**WHY:** The required outcome is: attempts resolve to the correct durable patient.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Attempts resolve to the correct durable patient. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Connect attempt detail and answer provenance

**ACTION:** Connect attempt detail and answer provenance.

**WHY:** The required outcome is: staff can distinguish spoken extraction from confirmed answers.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Staff can distinguish spoken extraction from confirmed answers. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Expose approved actions with revision conflict handling

**ACTION:** Expose approved actions with revision conflict handling.

**WHY:** The required outcome is: a stale page cannot overwrite newer work.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** A stale page cannot overwrite newer work. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Verify keyboard navigation and screen-reader status messages

**ACTION:** Verify keyboard navigation and screen-reader status messages.

**WHY:** The required outcome is: action success and failures are perceivable.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Action success and failures are perceivable. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Attempts resolve to the correct durable patient.
- [ ] Step 2: Staff can distinguish spoken extraction from confirmed answers.
- [ ] Step 3: A stale page cannot overwrite newer work.
- [ ] Step 4: Action success and failures are perceivable.

## 16. Error and failure semantics

Check identity linkage and backend response before adding UI-derived joins.
...
### TEST-UI-002-01: Role boundary

**Purpose:** Prove role boundary under the target contract.
...
**Action:** Exercise `Role boundary` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Hidden controls also fail at direct backend access. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Check identity linkage and backend response before adding UI-derived joins.

### TEST-UI-002-02: Late transcript

**Purpose:** Prove late transcript under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Late transcript` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Detail updates without reopening terminal lifecycle. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Check identity linkage and backend response before adding UI-derived joins.

### TEST-UI-002-03: Conflicting edits

**Purpose:** Prove conflicting edits under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Conflicting edits` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Staff sees409 and can reload without losing the other update. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Hidden controls also fail at direct backend access.
- [ ] Detail updates without reopening terminal lifecycle.
- [ ] Staff sees409 and can reload without losing the other update.
...
Disable affected actions and retain read-only authenticated detail.
