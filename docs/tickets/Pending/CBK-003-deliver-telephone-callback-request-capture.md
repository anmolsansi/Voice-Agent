# CBK-003: Deliver telephone callback request capture
...
**Depends on:** [CBK-001](CBK-001-model-standalone-callback-requests-independently.md), [TEL-002](TEL-002-verify-and-durably-receive-provider-events.md), [VOI-002](VOI-002-implement-approved-fixed-script-conversation-behavior.md)

**Blocks:** [CBK-005](CBK-005-add-callback-reporting-and-recovery-evidence.md)
...
Deliver telephone callback request capture. The finished outcome is defined by the contract and observable assertions below.

## 2. Junior engineer mental model

Inbound voice plumbing does not yet constitute a standalone callback-request service.

Use verified provider ingress and a fixed callback script to confirm contact/window and disclose clinic-hours limitations.
...
This ticket closes the gap through: deliver telephone callback request capture. Its downstream dependents must not assume the target behavior exists before the completion evidence is recorded.
...
Use verified provider ingress and a fixed callback script to confirm contact/window and disclose clinic-hours limitations. Caller ID alone is not verified identity. Persist request/work before telling caller it was received. Failed capture offers approved fallback. Never claim emergency dispatch or store call recordings.
...
Ticket-specific boundary: Use verified provider ingress and a fixed callback script to confirm contact/window and disclose clinic-hours limitations. Caller ID alone is not verified identity. Persist request/work before telling caller it was received. Failed capture offers approved fallback. Never claim emergency dispatch or store call recordings.

## 7. Facts, assumptions and unknowns

- **Fact:** Inbound voice plumbing does not yet constitute a standalone callback-request service.
- **Assumption:** The merged foundation remains the starting point; verify branch drift before editing.
- **Required prerequisite evidence:** CBK-001, TEL-002, VOI-002
...
Deliver deliver telephone callback request capture and the exact tests/checkpoints below. Modify adjacent shared contracts only when required, with affected-ticket coordination.
...
- `tests/phone-callback.integration.test.cjs`
...
### Step 1: Route inbound callback purpose without schedule fabrication

**ACTION:** Route inbound callback purpose without schedule fabrication.

**WHY:** The required outcome is: unknown callers can request help safely.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Unknown callers can request help safely. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.
...
### Step 2: Confirm callback number and window

**ACTION:** Confirm callback number and window.

**WHY:** The required outcome is: misrecognition is corrected before durable receipt.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Misrecognition is corrected before durable receipt. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 3: Create idempotent request/work transaction

**ACTION:** Create idempotent request/work transaction.

**WHY:** The required outcome is: provider reconnect/replay cannot duplicate requests.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** Provider reconnect/replay cannot duplicate requests. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

### Step 4: Implement failure and urgent-language guidance

**ACTION:** Implement failure and urgent-language guidance.

**WHY:** The required outcome is: no unsupported clinical promise is made.. Without this checkpoint, the next step cannot safely rely on the result.

**VERIFY:** No unsupported clinical promise is made. Capture the relevant response/state diff, decision artifact or test output with the commit/environment; redact sensitive values.

**IF IT FAILS:** Stop dependent work, inspect the failure guide below and correct the cause. Keep the failed result in the evidence record rather than replacing it with an assumed pass.

## 15. Progress checkpoints

- [ ] Step 1: Unknown callers can request help safely.
- [ ] Step 2: Misrecognition is corrected before durable receipt.
- [ ] Step 3: Provider reconnect/replay cannot duplicate requests.
- [ ] Step 4: No unsupported clinical promise is made.

## 16. Error and failure semantics

Inspect confirmed structured contact and transaction outcome rather than raw transcript guesses.
...
### TEST-CBK-003-01: Unknown caller

**Purpose:** Prove unknown caller under the target contract.
...
**Action:** Exercise `Unknown caller` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Request persists without invented patient. Assert the response or artifact, durable state and audit/side effects separately.
...
**Failure interpretation:** Inspect confirmed structured contact and transaction outcome rather than raw transcript guesses.

### TEST-CBK-003-02: Provider replay

**Purpose:** Prove provider replay under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Provider replay` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** One logical callback request. Assert the response or artifact, durable state and audit/side effects separately.

**Why it matters:** This assertion is the boundary that distinguishes the intended outcome from a plausible but incomplete success.

**Failure interpretation:** Inspect confirmed structured contact and transaction outcome rather than raw transcript guesses.

### TEST-CBK-003-03: Database failure

**Purpose:** Prove database failure under the target contract.

**Level:** Choose the lowest meaningful layer; use real PostgreSQL for persistence/concurrency and a rendered browser for UI behavior.

**Setup:** Use a clean synthetic fixture or isolated approval record. Establish the required prerequisite states, user/role, current revision and persisted baseline. For negative cases, seed the contrary permission/expiry/outage state explicitly; never change a real clinic record.

**Action:** Exercise `Database failure` through the actual changed entrypoint. Where the scenario names concurrency, issue competing requests together; where it names restart/outage, interrupt and restore the relevant dependency rather than mocking a successful response.

**Expected assertions:** Caller is not told request was saved; fallback is offered. Assert the response or artifact, durable state and audit/side effects separately.
...
- [ ] Request persists without invented patient.
- [ ] One logical callback request.
- [ ] Caller is not told request was saved; fallback is offered.
...
Route telephone entry to approved staffed contact/fallback and retain queued requests.
...
Notify the implementation owner of CBK-005 using the agreed project workflow after evidence review. This document does not itself authorize external messages, issue creation, merge or deployment. Update linked contract versions and note any remaining limitations.
