...
const CONSENT_CHECKBOX_FIELDS: Array<{
  ...
  { key: 'treatmentConsent', name: 'consent.treatmentConsent', label: 'I consent to evaluation and treatment', required: true, description: 'Required for MVP submission. This consent must be checked before intake can be submitted.' },
  { key: 'hipaaAcknowledgment', name: 'consent.hipaaAcknowledgment', label: 'I acknowledge receipt/availability of HIPAA privacy practices', required: true, description: 'Required for MVP submission. This acknowledgment confirms the patient can access the privacy practices notice.' },
  { key: 'financialResponsibility', name: 'consent.financialResponsibility', label: 'I accept financial responsibility for services rendered', description: 'Optional for MVP, but included in the frozen schema for this section.' },
];
...
export function ConsentSection({ values, sectionState, saveState, onFieldChange }: ConsentSectionProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <FormSection title="Review and sign" description="Capture the frozen MVP consent acknowledgments and signature only. The signed timestamp is generated by the backend when the signature name is saved.">
        <div className="grid gap-4">
          ...
          <TextField name="consent.signatureName" label="Signature (full name)" required hint="Type the patient’s full name as an acknowledgment signature for MVP." placeholder="Jordan Lee" value={values.signatureName} error={getRequiredTextError(values.signatureName)} onChange={handleSignatureChange} />
