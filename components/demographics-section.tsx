...
const REQUIRED_FIELD_KEYS: Array<keyof DemographicsValues> = [
  'firstName',
  'lastName',
  'dateOfBirth',
  'sexAtBirth',
...
const BASIC_FIELDS: FieldDefinition[] = [
  ...
  },
  {
    ...
    placeholder: 'Woman, man, nonbinary, etc.',
    hint: 'Optional free-text entry for how you identify.',
  },
  {
    key: 'phone',
    label: 'Phone number',
  ...
  },
  {
    ...
    hint: 'Optional, but helpful for follow-up instructions.',
  },
];
...
const ADDRESS_FIELDS: FieldDefinition[] = [
  ...
  },
  {
    ...
    hint: 'Shown once any address detail is entered.',
  },
  {
    key: 'postalCode',
    label: 'ZIP code',
...
const initialValues: DemographicsValues = {
  firstName: '',
  lastName: '',
  dateOfBirth: '',
  sexAtBirth: '',
  sexAtBirthSelfDescribe: '',
  genderIdentity: '',
  phone: '',
  email: '',
  addressLine1: '',
...
export function DemographicsSection() {
  ...
  function updateField<Key extends keyof DemographicsValues>(key: Key, value: DemographicsValues[Key]) {
    setValues((current) => ({
      ...current,
      [key]: value,
      ...(key === 'sexAtBirth' && value !== 'self_describe' ? { sexAtBirthSelfDescribe: '' } : {}),
    }));
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <FormSection
        ...
        description="Collect the core contact and identity details needed to begin the visit. This stays local to the patient intake shell for now, without backend save wiring."
      >
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <DateField
            ...
            label="Date of birth"
            required
            hint="Must be a valid past date. Server-side validation remains the source of truth."
            value={values.dateOfBirth}
            error={getRequiredError(values.dateOfBirth, true)}
            onChange={handleTextChange('dateOfBirth')}
            max={new Date().toISOString().split('T')[0]}
          />

          <SelectField
            name="patient.sexAtBirth"
            label="Sex at birth"
            required
            hint="MVP enum values follow the shared schema freeze."
            options={sexAtBirthOptions}
            value={values.sexAtBirth}
            error={getRequiredError(values.sexAtBirth, true)}
            onChange={handleTextChange('sexAtBirth')}
          />

          {values.sexAtBirth === 'self_describe' ? (
            <TextField
              name="patient.sexAtBirthSelfDescribe"
              label="Describe sex at birth"
              hint="Shown only when self describe is selected above."
              placeholder="Describe in your own words"
              value={values.sexAtBirthSelfDescribe}
              onChange={handleTextChange('sexAtBirthSelfDescribe')}
      ...
      <FormSection
        title="Address"
        description="Address details are optional in MVP. State and ZIP appear once address entry begins so the section does not feel heavier than necessary."
      >
        <div className="grid gap-5 md:grid-cols-2">
          ...
          {addressInUse ? (
            <>
              {ADDRESS_FIELDS.slice(3).map((field) => (
                <TextField
                  key={field.key}
                  name={`patient.${field.key}`}
                  label={field.label}
            ...
            <div className="rounded-3xl border border-dashed border-slate-300 bg-slate-50 px-5 py-6 text-sm leading-6 text-slate-500 md:col-span-2">
              Add an address if you want staff to have mailing details on file. State and ZIP will appear after you begin entering the address.
            </div>
          )}
        </div>
      </FormSection>
